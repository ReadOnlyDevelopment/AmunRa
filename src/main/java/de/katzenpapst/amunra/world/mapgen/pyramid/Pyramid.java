...
public class Pyramid extends BaseStructureStart {
	...
	/*
	 * protected void initRooms() { for(int i=0;i<12;i++) { PyramidRoom room = new PyramidRoom(); room.setParent(this); StructureBoundingBox roomBB = this.getSmallRoomBB(i+1); StructureBoundingBox entranceBB = this.getRoomEntranceBox(i+1, roomBB); room.setBoundingBoxes(entranceBB, roomBB); //room.setStructureBoundingBox(roomBB); //room.setEntranceBB(entranceBB); roomList[i] = room;
	 * 
	 * 
	 * }
	 * 
	 ...
	 * 
	 * StructureBoundingBox innerRoomBB = new StructureBoundingBox( this.structBB.minX+innerRoomTotalOffset, this.structBB.minZ+innerRoomTotalOffset, this.structBB.maxX-innerRoomTotalOffset, this.structBB.maxZ-innerRoomTotalOffset );
	 * 
	 * 
	 * 
	 * StructureBoundingBox mainEntranceBB = new StructureBoundingBox();
	 * 
	 * mainEntranceBB.minX = innerRoomBB.getCenterX()-1; mainEntranceBB.maxX = innerRoomBB.getCenterX()+1; mainEntranceBB.minZ = innerRoomBB.maxZ+1; mainEntranceBB.maxZ = innerRoomBB.maxZ+4;
	 * 
	 * centralRoom = new PyramidRoom(); centralRoom.setBoundingBoxes(innerRoomBB, mainEntranceBB); centralRoom.setParent(this); }
	 */
				...
				for (int y = 0; y <= radius; y++) {
					...
					if (y >= 5 && y <= 13) {
						...
						if (x >= xCenter - 4 && x <= xCenter + 4) {
							...
							// cut in the tunnel
							if (x >= xCenter - 1 && x <= xCenter + 1 && z >= startZ + 6 && y >= 5 && y <= 6 + tunnelHeight && z >= startZ + 5 && z <= startZ + 5 + innerRingOffset - tunnelWidth
						...
						if (y >= 5 && y <= 6 + tunnelHeight && x >= startX + innerRingOffset && x <= stopX - innerRingOffset && z >= startZ + innerRingOffset && z <= stopZ - innerRingOffset) {
	...
	protected StructureBoundingBox getSmallRoomBB(int position) {
		...
		/*
		 * Z ^ | +-----------+ +-----------+ | +--+ +--+ | | +--+ +--+ | | |10| |9 | | | |8 | |7 | | | +--+ +--+ | | +--+ +--+ | | +--+ +----+ +----+ +--+ | | |11| | +-----+ | |6 | | | +--+ | | | | +--+ | | +--+ | | | | +--+ | | |12| | +-+ +-+ | |5 | | | +--+ +-----------+ +--+ | | +--+ +--+ +--+ +--+ | | |1 | |2 | |3 | |4 | | | +--+ +--+ +--+ +--+ | +-------------------------+----> X
		 */
	...
	public void setMainRoom(PyramidRoom room) {
		...
		/*
		 * x >= xCenter-1 && x <= xCenter+1 && (y >= 5 && y <= 6+tunnelHeight) && (z <= stopZ-innerRingOffset-tunnelWidth && z >= stopZ-innerRoomTotalOffset)
		 */
		StructureBoundingBox mainEntranceBB = new StructureBoundingBox();
