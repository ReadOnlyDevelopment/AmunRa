...
public class GridVillageStart extends BaseStructureStart {
	...
	protected void drawGrid(int chunkX, int chunkZ, Block[] arrayOfIDs, byte[] arrayOfMeta) {
		...
		/*
		 * StructureBoundingBox chunkBB = CoordHelper.getChunkBB(chunkX, chunkZ); int minGroundLevel = this.getAverageGroundLevel(arrayOfIDs, arrayOfMeta, structBB, chunkBB, -1);
		 */
	...
	/**
	 * From what I understood, this is called first, then, after this is done, populateChunk is called and after that, the chunk is done and won't be touched again
	 *
	 * @param chunkX
	 * @param chunkZ
	 * @param arrayOfIDs
	...
	public void setComponents(ArrayList components) {
		...
		for (Object comp : components) {
			...
			StructureBoundingBox componentBox = new StructureBoundingBox(structBB.minX + effectiveGridSize * gridX + 2, structBB.minZ + effectiveGridSize * gridZ + 2, structBB.minX + effectiveGridSize * gridX + 1 + this.gridSize, structBB.minZ + effectiveGridSize * gridZ + 1 + this.gridSize);
