		...
		for (int x = startX; x <= stopX; x++) {
			for (int z = startZ; z <= stopZ; z++) {
				...
				for (int y = 0; y <= radius; y++) {
					if (x >= startX + y && x <= stopX - y && (z == startZ + y || z == stopZ - y) || (x == startX + y || x == stopX - y) && z >= startZ + y && z <= stopZ - y) {
						placeBlockRel2BB(blocks, metas, chunkX, chunkZ, x, groundLevel + y + 1, z, wall);
					}
					/*
					 * if((x >= startX+y && x <= stopX-y) && (z >= startZ+y && z <= stopZ-y)) { placeBlockRel2BB(blocks, metas, chunkX, chunkZ, x, groundLevel+y+1, z, wall); } if((x >= startX+y && x <= stopX-y-1) && (z >= startZ+y+1 && z <= stopZ-y-y)) { placeBlockRel2BB(blocks, metas, chunkX, chunkZ, x, groundLevel, z, air); }
					 */
				}
