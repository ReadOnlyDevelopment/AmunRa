...
abstract public class BaseStructureComponent {
	...
	/**
	 * Places a block into the arrays using absolute coordinates. Assumes the chunk the coordinates are in is to be edited. Block/meta version
	 *
	 * @param blocks
	 * @param metas
	 * @param x
	...
	/**
	 * Places a block into the arrays using absolute coordinates. Assumes the chunk the coordinates are in is to be edited. BlockMetaPair version
	 *
	 * @param blocks
	 * @param metas
	 * @param x
	...
	/**
	 * Places a block into the arrays using absolute coordinates+coordinates of the current chunk. If the coordinates are not inside the given chunk, nothing happens. Block/meta version
	 *
	 * @param blocks
	 * @param metas
	 * @param x
	...
	/**
	 * Places a block into the arrays using absolute coordinates+coordinates of the current chunk. If the coordinates are not inside the given chunk, nothing happens. BlockMetaPair version
	 *
	 * @param blocks
	 * @param metas
	 * @param x
	...
	/**
	 * For doors, in a sense, and furnaces 1 0-+-2 3
	 *
	 * @param unrotated
	 * @param coordMode
	 * @return
	...
	/**
	 * This should work for stairs 0 = E 1 = W 2 = S 3 = N for torches, add +1; 0 means nothing and 5 means "on the ground", and the interpretation is "torch FACING dir" 3 1-+-0 2
	 *
	 * @param unrotated
	 * @param coordMode
	 * @return
	...
	/**
	 * Rotates the metadata which most things seem to use: 0 2-+-3 1 This should work for solar collectors and trapdoors, but in a reversed non-intuitive way
	 *
	 * @param unrotated
	 * @param coordMode
	 * @return
	 ...
	 * @param chunkBB
	 * @param box
	 * @param block   / protected boolean drawArea(Block[] blocks, byte[] metas, StructureBoundingBox chunkBB, StructureBoundingBox box, BlockMetaPair block) { StructureBoundingBox actualBox = intersectBoundingBoxes(chunkBB, box); if(actualBox == null) { return false; } for(int x=actualBox.minX; x<=actualBox.maxX; x++) { for(int y=actualBox.minY; y<=actualBox.maxY; y++) { for(int z=actualBox.minZ; z<=actualBox.maxZ; z++) { int xOffset = getXWithOffset(x, z); int zOffset = getZWithOffset(x, z);
	 *                int relX = CoordHelper.abs2rel(xOffset); int relZ = CoordHelper.abs2rel(zOffset); placeBlockRel(blocks, metas, relX, y, relZ, block); } } } return true; }
	 */
	 ...
	 */
	/*
	 * protected void spawnEntity(Class<? extends EntityLiving> entityToSpawn, int x, int z) { EntityLiving ent = null; try { ent = entityToSpawn.getConstructor(World.class).newInstance(this.parent.getWorld()); } catch (Throwable e) { e.printStackTrace(); return; }
	 * 
	 * ent.onSpawnWithEgg(null);// NO IDEA int xOffset = getXWithOffset(x, z); //y = getYWithOffset(y); int zOffset = getZWithOffset(x, z); this.parent.spawnLater(ent, xOffset, groundLevel, zOffset); }
	 */
