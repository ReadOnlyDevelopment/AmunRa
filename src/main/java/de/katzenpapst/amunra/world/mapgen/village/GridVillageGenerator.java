...
public class GridVillageGenerator extends StructureGenerator {
	/*
	 * protected class ComponentEntry { public Class<? extends GridVillageComponent> clazz; public float probability; public int minAmount; public int maxAmount;
	 * 
	 * public ComponentEntry(Class<? extends GridVillageComponent> clazz, float probability, int minAmount, int maxAmount) { this.clazz = clazz; this.probability = probability; this.minAmount = minAmount; this.maxAmount = maxAmount; } }
	 */
	...
	protected boolean canGenerateHere(int chunkX, int chunkZ, Random rand) {
		...
		this.rand.setSeed(this.world.getSeed() ^ this.getSalt() ^ superchunkX ^ superchunkZ);
		...
		/*
		 * final long randX = chunkX * getSalt(); final long randZ = chunkZ * getSalt(); this.rand.setSeed(randX ^ randZ ^ this.world.getSeed()); return this.rand.nextInt(700) == 0;
		 */
	}
	...
	protected BaseStructureStart createNewStructure(int xChunkCoord, int zChunkCoord) {
		...
		Random rand4structure = new Random(this.world.getSeed() ^ this.getSalt() ^ xChunkCoord ^ zChunkCoord);

		GridVillageStart start = new GridVillageStart(this.world, xChunkCoord, zChunkCoord, rand4structure);
		ArrayList compList = generateSubComponents(components, rand4structure, 0);
		/*
		 * ArrayList compList = new ArrayList(); // now prepare the actual component list for(SubComponentData entry: components) { try { // generate the minimum amount GridVillageComponent cmp = null; int nrGenerated = 0; boolean shouldGenerateMore = true;
		 * 
		 * // now generate the extra while(shouldGenerateMore) { shouldGenerateMore = false; if(entry.minAmount > 0 && nrGenerated < entry.minAmount) { shouldGenerateMore = true; } else { if(rand4structure.nextFloat() < entry.probability) { shouldGenerateMore = true; } }
		 * 
		 * if(shouldGenerateMore) { cmp = (GridVillageComponent) entry.clazz.getConstructor().newInstance(); compList.add(cmp); // start.addComponent(cmp); nrGenerated++; } if(nrGenerated >= entry.maxAmount) { break; } }
		 * 
		 * } catch (Throwable e) { FMLLog.info("Instantiating "+entry.clazz.getCanonicalName()+" failed"); e.printStackTrace(); } }
		 */
