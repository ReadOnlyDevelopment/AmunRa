...
public class Volcano extends BaseStructureStart {
	...
	public Volcano(World world, int chunkX, int chunkZ, Random rand) {
		...
		StructureBoundingBox bb = new StructureBoundingBox(startX - radius, startZ - radius, startX + radius, startZ + radius);
	...
	public boolean generateChunk(int chunkX, int chunkZ, Block[] blocks, byte[] metas) {
		...
		for (int x = myBB.minX; x <= myBB.maxX; x++) {
			for (int z = myBB.minZ; z <= myBB.maxZ; z++) {
				...
				int lowestBlock = BaseStructureComponent.getHighestSpecificBlock(blocks, metas, CoordHelper.abs2rel(x, chunkX), CoordHelper.abs2rel(z, chunkZ), this.mountainMaterial.getBlock(), this.mountainMaterial.getMetadata());
				if (lowestBlock == -1) {
				...
				if (sqDistance <= sqRadius) {
					...
					if (distance <= this.shaftRadius) {
						...
						if (distance > faloffRadius && lowestBlock < height && groundLevel > lowestBlock) {
							...
							double faloffFactor = (distance - faloffRadius) / this.falloffWidth;
					...
					if (distance < this.shaftRadius + 2) {
						for (int y = maxDepth + 1; y < height; y++) {
							...
							if (distance <= this.shaftRadius) {
								BaseStructureComponent.placeBlockAbs(blocks, metas, x, y, z, chunkX, chunkZ, fluid);
							} else {
								// if(y == groundLevel+height-1) {
								// this.placeBlockAbs(blocks, metas, x, y, z, chunkX, chunkZ, fluid);
								// } else {
								BaseStructureComponent.placeBlockAbs(blocks, metas, x, y, z, chunkX, chunkZ, this.shaftMaterial);
								// }
							}
						}
						...
						for (int y = lowestBlock; y < height; y++) {
							...
							BaseStructureComponent.placeBlockAbs(blocks, metas, x, y, z, chunkX, chunkZ, mountainMaterial);
				...
				if (hasMagmaChamber) {
					...
					for (int y = 0; y < this.magmaChamberHeight; y++) {
						if (xRel * xRel / magmaChamberWidth * magmaChamberWidth + y * y / magmaChamberHeight * magmaChamberHeight + zRel * zRel / magmaChamberWidth * magmaChamberWidth <= 1

						) {
							BaseStructureComponent.placeBlockAbs(blocks, metas, x, y + maxDepth, z, chunkX, chunkZ, fluid);
						}
					}
	...
	protected double getHeightFromDistance(double distance) {
		return maxHeight * ((this.radius - distance) / this.radius);
	}
