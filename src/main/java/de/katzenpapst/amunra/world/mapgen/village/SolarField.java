...
public class SolarField extends GridVillageComponent {
			...
			for (int z = startZ; z < stopZ; z++) {
				...
				if (x == startX + 2 || x == stopX - 3) {
					if (z == startZ + 2) {
						...
						/*
						 * if(placeBlockRel2BB(blocks, metas, chunkX, chunkZ, x, groundLevel, z, GCBlocks.solarPanel, this.rotateStandardMetadata(1, this.coordMode))) { this.parent.addPopulator(new TouchSolarPanel(getXWithOffset(x, z), groundLevel, getZWithOffset(x, z))); }
						 */
					...
					int storageMetadata = BaseStructureComponent.rotateStandardMetadata(2, this.coordMode);
	...
	private void placeSolarPanel(Block[] blocks, byte[] metas, int chunkX, int chunkZ, int x, int y, int z, int meta) {
		int rotationMetadata = BaseStructureComponent.rotateStandardMetadata(meta, this.coordMode);
		if (AmunRa.config.villageAdvancedMachines) {
