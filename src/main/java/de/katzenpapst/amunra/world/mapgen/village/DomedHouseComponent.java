				...
				if (houseRadiusSq + 1 >= wallRadiusSq) {
					...
					for (int y = 0; y < wallHeight; y++) {
						if (houseRadiusSq - 1 <= wallRadiusSq) {
							...
							// also torches
							/*
							 * if(x == startX+1 && z == zCenter && y == 2) { placeBlockRel2BB(blocks, metas, chunkX, chunkZ, x, groundLevel+y, z, GCBlocks.glowstoneTorch, rotateTorchMetadata(1, this.coordMode)); } else if(x == stopX-1 && z == zCenter && y == 2) { placeBlockRel2BB(blocks, metas,chunkX, chunkZ, x, groundLevel+y, z, GCBlocks.glowstoneTorch, rotateTorchMetadata(2, this.coordMode)); // } else if(z == startZ+1 && x == xCenter && y == 2) { placeBlockRel2BB(blocks, metas,chunkX, chunkZ,
							 * x, groundLevel+y, z, GCBlocks.glowstoneTorch, rotateTorchMetadata(3, this.coordMode)); // rotate to -z? } else if(z == stopZ-1 && x == xCenter && y == 2) { placeBlockRel2BB(blocks, metas,chunkX, chunkZ, x, groundLevel+y, z, GCBlocks.glowstoneTorch, rotateTorchMetadata(4, this.coordMode)); // rotate to -z? } else { placeBlockRel2BB(blocks, metas, chunkX, chunkZ, x, groundLevel+y, z, air); }
							 */
							placeBlockRel2BB(blocks, metas, chunkX, chunkZ, x, groundLevel + y, z, air);

							/*
							 * if(y==0 && x == startX+1 && z == startZ+1) { // random crafting table placeBlockRel2BB(blocks, metas,chunkX, chunkZ, x, groundLevel+y, z, Blocks.crafting_table, 0); }
							 */
						}
					}
