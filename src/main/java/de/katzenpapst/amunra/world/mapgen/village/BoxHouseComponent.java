		...
		for (int x = startX; x <= stopX; x++) {
			for (int z = startZ; z <= stopZ; z++) {
				...
				if (x == xCenter && z == zCenter) {
					...
					/*
					 * EntityCreature villager = new EntityRobotVillager(this.parent.getWorld()); villager.onSpawnWithEgg(null);// NO IDEA int xOffset = getXWithOffset(x, z); //y = getYWithOffset(y); int zOffset = getZWithOffset(x, z); this.parent.spawnLater(villager, xOffset, groundLevel, zOffset);
					 */
				}
