...
public class AmunRaAsteroidsChunkProvider extends ChunkProviderGenerate {
	...
	public AmunRaAsteroidsChunkProvider(World par1World, long par2, boolean par4) {
		...
		asteroidStoneBlocks = new BlockMetaPair[] { new BlockMetaPair(AsteroidBlocks.blockBasic, (byte) 0), new BlockMetaPair(AsteroidBlocks.blockBasic, (byte) 1), new BlockMetaPair(AsteroidBlocks.blockBasic, (byte) 2) };
	...
	protected int getCountInNegXDirection(IChunkProvider world, int x, int y, int z, int maxLength, boolean isAir) {
		...
		for (int i = 0; i < maxLength; i++) {
			if (world.getBlock(x - i - 1, y, z) instanceof BlockAir == isAir) {
				count++;
			} else {
				break;
			}
	...
	protected void initBlockTypes() {
		...
		generatorsTrees = new WorldGenData[] { new WorldGenData(new WorldGenTreeBySapling(false, 5, ARBlocks.blockMethaneSapling), 2) };
		generatorsGrass = new WorldGenData[] { new WorldGenData(new WorldGenTallgrassMeta(ARBlocks.blockMethaneTGrass), 2) };
		generatorsLakes = new WorldGenData[] { new WorldGenData(new WorldGenLakes(Blocks.water), 2), new WorldGenData(new WorldGenLakes(Blocks.lava), 2) };
	...
	public void populate(IChunkProvider par1IChunkProvider, int chunkX, int chunkZ) {
		...
		this.world.getBiomeGenForCoords(x + 16, z + 16);
		...
		if (this.rand.nextBoolean()) {
			...
			for (int i = 0; i < numOfBlocks; i++) {
				...
				if (y0 == y / 16 % 2) {
					...
					world.setBlock(px, y, pz, block, meta, 2);
					// this does something about the light
					// so, the more blocks towards -x are NOT air, the brighter are we?!
					int count = 7;
					if (!(world.getBlock(px - 1, y, pz) instanceof BlockAir)) {
						count = 1;// 0
					} else if (!(world.getBlock(px - 2, y, pz) instanceof BlockAir)) {
						count = 3;// 1
					} else if (!(world.getBlock(px - 3, y, pz) instanceof BlockAir)) {
						count = 5;// 2
					} else if (!(world.getBlock(px - 4, y, pz) instanceof BlockAir)) {
						count = 6;// 3
					}
					/*
					...
					world.setLightValue(EnumSkyBlock.Block, px - 1, y, pz, count);// +1
				}
			}
		}
				...
				for (WorldGenData data : generatorsTrees) {
					if (rand.nextInt(data.chance) == 0) {
						...
						for (int tries = 0; tries < 5; tries++) {
							...
							if (data.worldGen.generate(world, rand, i, this.getTerrainHeightAt(i - x, k - z, sizeYArray, xMin, zMin, zSize, asteroidY, asteroidSize), k)) {
								break;
							}
						}
						treesdone = true;
				...
				if (!treesdone || rand.nextInt(TALL_GRASS_CHANCE) == 0) {
					for (WorldGenData data : generatorsGrass) {
						if (rand.nextInt(data.chance) == 0) {
							int i = rand.nextInt(16) + x + 8;
							int k = rand.nextInt(16) + z + 8;
							data.worldGen.generate(world, rand, i, this.getTerrainHeightAt(i - x, k - z, sizeYArray, xMin, zMin, zSize, asteroidY, asteroidSize), k);
						}
					}
				}
				/*
				 * if(rand.nextInt(FLOWER_CHANCE) == 0) { int i = rand.nextInt(16) + x + 8; int k = rand.nextInt(16) + z + 8; new WorldGenFlowers(this.FLOWER).generate(world, rand, i, this.getTerrainHeightAt(i - x, k - z, sizeYArray, xMin, zMin, zSize, asteroidY, asteroidSize), k); }
				 */
				// lakes
		...
		for (int xx = 0; xx < 16; xx++) {
			...
			for (int zz = 0; zz < 16; zz++) {
				...
				for (int y = 16; y < 240; y++) {
					world.updateLightByType(EnumSkyBlock.Block, xPos, y, zPos);
				}
			}
		}
...
//        long time2 = System.nanoTime();
		final Chunk curChunk = new Chunk(this.world, ids, meta, chunkX, chunkZ);
