...
public class EntityLaserArrow extends EntityBaseLaserArrow {
	...
	protected void onImpactBlock(World world, int x, int y, int z) {
		...
		if (smeltResult != null) {
			...
			if (blockId > 0) {
				...
				if (b != Blocks.air) {
					 ...
					 */
					world.setBlock(x, y, z, b, smeltResult.getItemDamage(), 3);
					return;
				}
			}
		}
		if (OxygenUtil.noAtmosphericCombustion(world.provider)) {

			if (OxygenUtil.isAABBInBreathableAirBlock(world, AxisAlignedBB.getBoundingBox(x, y, z, x + 1, y + 1, z + 1))) {
				WorldHelper.setFireToBlock(world, x, y, z, posX, posY, posZ);
			}
		} else {
			WorldHelper.setFireToBlock(world, x, y, z, posX, posY, posZ);
		}
		...
		/*
		 * if(world.getBlock(x, y+1, z) == Blocks.air) { //OxygenUtil.isAABBInBreathableAirBlock(world, bb) // no oxygen check for now world.setBlock(x, y+1, z, Blocks.fire, 0, 3); }
		 */
	...
	protected void setFireToBlock(World world, int x, int y, int z) {
		// omg
