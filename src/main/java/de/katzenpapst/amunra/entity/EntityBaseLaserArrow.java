...
abstract public class EntityBaseLaserArrow extends Entity implements IProjectile, IThrowableEntity, IAntiGrav {
	...
	protected void onImpactBlock(World world, int xTile2, int yTile2, int zTile2) {
	...
	protected void onImpactEntity(MovingObjectPosition mop) {
		if (this.doesFireDamage() && !(mop.entityHit instanceof EntityEnderman)) {
			// hm
			if (OxygenUtil.noAtmosphericCombustion(mop.entityHit.world.provider)) {
				// usually, stuff doesn't burn here
				if (!OxygenUtil.isAABBInBreathableAirBlock(mop.entityHit.world, mop.entityHit.boundingBox, false))
	...
	/*
	 * public boolean isHot() { return this.dataWatcher.getWatchableObjectInt(16) == 1; }
	 * 
	 * public void setHot(boolean isHot) { this.dataWatcher.updateObject(16, isHot ? 1 : 0); }
	 */
	...
	public void onUpdate() {
		...
		if (!block.isAir(this.world, this.xTile, this.yTile, this.zTile)) {
			block.setBlockBoundsBasedOnState(this.world, this.xTile, this.yTile, this.zTile);
			AxisAlignedBB axisalignedbb = block.getCollisionBoundingBoxFromPool(this.world, this.xTile, this.yTile, this.zTile);
		...
		if (this.inGround) {
			...
			MovingObjectPosition movingobjectposition = this.world.func_147447_a(vec3, vec31, false, true, false);
			...
			Entity entity = null;
			List<Entity> list = this.world.getEntitiesWithinAABBExcludingEntity(this, this.boundingBox.addCoord(this.motionX, this.motionY, this.motionZ).expand(1.0D, 1.0D, 1.0D));
			double d0 = 0.0D;
			int l;
			float f1;
			...
			if (movingobjectposition != null) {
				if (movingobjectposition.entityHit != null) {
					...
					if (movingobjectposition.entityHit.attackEntityFrom(damagesource, dmgValue)) {
						...
						this.setDead();
						/*
						 * // reflexion? this.motionX *= -0.10000000149011612D; this.motionY *= -0.10000000149011612D; this.motionZ *= -0.10000000149011612D; this.rotationYaw += 180.0F; this.prevRotationYaw += 180.0F; this.ticksInAir = 0;
						 */
					}
					// ASD END
				} else {
					...
					if (!this.inTile.isAir(this.world, this.xTile, this.yTile, this.zTile)) {
						if (!this.world.isRemote) {
							this.onImpactBlock(this.world, this.xTile, this.yTile, this.zTile);
						}
						this.inTile.onEntityCollidedWithBlock(this.world, this.xTile, this.yTile, this.zTile, this);
					}
				}
			}
