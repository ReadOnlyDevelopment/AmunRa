...
public class EntityShuttle extends EntityTieredRocket {
	...
	public static void toCelestialSelection(EntityPlayerMP player, GCPlayerStats stats, int tier, boolean useFakeEntity) {
		...
		if (useFakeEntity) {
			Entity fakeEntity = new EntityShuttleFake(player.world, player.posX, player.posY, player.posZ, 0.0F);
			player.world.spawnEntityInWorld(fakeEntity);
	...
	protected void checkStandingPosition() {
		...
		if (this.getLandingPad() != null) {
			...
			if (dockPosition != null) {
				TileEntity tile = world.getTileEntity(dockPosition.x, dockPosition.y, dockPosition.z);
				if (tile != null) {
			...
			Vector3int highest = WorldHelper.getHighestNonEmptyBlock(world, bX - 1, bX + 1, bY - 5, bY, bZ - 1, bZ + 1);

			if (highest != null) {
				TileEntity tileBelow = world.getTileEntity(highest.x, highest.y, highest.z);
	...
	protected boolean isSafeForPlayer(double x, double y, double z) {
		...
		return WorldHelper.isNonSolid(world, (int) x, y1, (int) z) && WorldHelper.isNonSolid(world, (int) x, y1 + 1, (int) z) && WorldHelper.isSolid(world, (int) x, y1 - 1, (int) z, true);
	}
	...
	public void onReachAtmosphere() {
		...
		if (this.riddenByEntity != null && !this.world.isRemote) {
	...
	protected void tryFindAnotherDock() {
		Vector3int dock = ShuttleDockHandler.findAvailableDock(this.world.provider.dimensionId);
		if (dock != null) {
	...
	protected void tryToDock() {
		...
		if (world.getChunkProvider().chunkExists(chunkx, chunkz)) {
			...
			TileEntity te = targetVec.getTileEntity(world);
