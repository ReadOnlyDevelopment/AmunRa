...
import de.katzenpapst.amunra.old.block.BlockStairsAR;
import de.katzenpapst.amunra.old.block.ore.BlockOreMulti;
import de.katzenpapst.amunra.old.block.ore.SubBlockOre;
...
public class RecipeHelper {
	...
	protected static void addCircuitFabricatorRecipe(ItemStack output, ItemStack[] crystal, ItemStack[] silicon1, ItemStack[] silicon2, ItemStack[] redstone, ItemStack[] optional) {
					...
					for (ItemStack element4 : redstone) {
						...
						if (optional.length > 0) {
							for (ItemStack element5 : optional) {
								addCircuitFabricatorRecipeInternal(output, element, element2, element3, element4, element5);
							}
						} else {
		...
		// * Optional slot
		/*
		 * new ItemStack(Items.diamond), silicon, silicon, new ItemStack(Items.redstone), new ItemStack(Items.ender_pearl)
		 */
	}
	...
	protected static void addCircuitFabricatorRecipeInternal(ItemStack output, ItemStack crystal, ItemStack silicon1, ItemStack silicon2, ItemStack redstone, ItemStack optional) {
		if (optional != null) {
			...
			CircuitFabricatorRecipes.addRecipe(output, new ItemStack[] { crystal, silicon1, silicon2, redstone, optional });
		} else {
	...
	private static void addRaygunRecipe(ItemStack gun, ItemStack[] batteries, Object... recipe) {
		...
		for (ItemStack battery : batteries) {
			...
			// the gun itself
			/*
			 * ItemStack gunWithBattery = gun.copy(); ((ItemAbstractRaygun) gunWithBattery.getItem()).setUsedBattery(gunWithBattery, battery); gunWithBattery.setItemDamage(OreDictionary.WILDCARD_VALUE);
			 */
	...
	public static void addRocketRecipeWithChestPermutations(Item rocket, HashMap<Integer, ItemStack> input) {
		...
		/*
		 * ItemStack numChests0 = new ItemStack(rocket, 1, 0); ItemStack numChests1 = new ItemStack(rocket, 1, 1); ItemStack numChests2 = new ItemStack(rocket, 1, 2); ItemStack numChests3 = new ItemStack(rocket, 1, 3);
		 */
	 ...
	 */
	/*
	 * private static void initRaygunReloadingRecipes(ItemStack[] guns, ItemStack[] batteries) { for(ItemStack gun: guns) { for(ItemStack battery: batteries) { GameRegistry.addShapelessRecipe(gun, new Object[]{gun, battery}); } } }
	 */
	...
	public static void initRecipes() {
		...
		addCircuitFabricatorRecipe(enderWaferStack, new ItemStack(Items.diamond), siliconArray, siliconArray, new ItemStack(Items.redstone), new ItemStack(Items.ender_pearl));

		addCircuitFabricatorRecipe(lithiumMeshStack, lithiumGemStack, siliconArray, siliconArray, new ItemStack(Items.redstone), new ItemStack(Items.paper));

		addCircuitFabricatorRecipe(uranMeshStack, "ingotUranium", siliconArray, siliconArray, new ItemStack(Items.redstone), lithiumMeshStack);

		// *** compressing ***
		CompressorRecipes.addRecipe(ARItems.lightPlating.getItemStack(1), "XYX", "XYX", 'X', new ItemStack(GCItems.basicItem, 1, 8), // compressed alu
		...
		/*
		 * initRaygunReloadingRecipes(new ItemStack[]{ raygun, cryogun }, new ItemStack[]{ battery, liBattery, quBattery, enBattery, nuBattery });
		 */
		...
		// batteries
		// lithium battery
		GameRegistry.addRecipe(liBattery, new Object[] { " X ", "XBX", "XAX", 'X', compressedAluStack, 'A', enderWaferStack, 'B', lithiumMeshStack });

		// advanced battery
		GameRegistry.addRecipe(enBattery, new Object[] { " X ", "XBX", "XAX", 'X', compressedAluStack, 'A', enderWaferStack, 'B', Blocks.redstone_block });

		// nuclear battery
		GameRegistry.addRecipe(nuBattery, new Object[] { " X ", "XBX", "XAX", 'X', compressedAluStack, 'A', enderWaferStack, 'B', uranMeshStack });

		// laser diode
		GameRegistry.addRecipe(laserDiodeStack, new Object[] { "XXX", "ABC", "XXX", 'X', compressedAluStack, // 8 = metadata for compressed alu
				'A', Blocks.glass_pane, 'B', ARItems.rubyGem.getItemStack(1), 'C', beamCore });

		// cryo diode
		GameRegistry.addRecipe(cryoDiodeStack, new Object[] { "XXX", "ABC", "XXX", 'X', compressedAluStack, // 8 = metadata for compressed alu
				'A', Blocks.glass_pane, 'B', ARItems.coldCrystal.getItemStack(1), 'C', beamCore });

		// laser gun
		addRaygunRecipe(raygun, batteries, new Object[] { "XYZ", " AZ", "  B", 'X', laserDiodeStack, 'Y', enderWaferStack, 'Z', compressedSteelStack, 'A', button, 'B', battery });

		// cryo gun
		addRaygunRecipe(cryogun, batteries, new Object[] { "XYZ", " AZ", "  B", 'X', cryoDiodeStack, 'Y', enderWaferStack, 'Z', compressedSteelStack, 'A', button, 'B', battery });

		// multitool
		addRaygunRecipe(nanotool, batteries, new Object[] { "NCN", " H ", " B ", 'N', ARItems.naniteCluster.getItemStack(1), 'C', ARItems.naniteControl.getItemStack(1), 'H', compressedTitaniumStack, 'B', battery });

		// my crafter
		GameRegistry.addRecipe(ARBlocks.getItemStack(ARBlocks.blockWorkbench, 1), "XX ", "XX ", "   ", 'X', compressedIronStack);

		// hydroponic
		GameRegistry.addRecipe(ARBlocks.getItemStack(ARBlocks.blockHydro, 2), "GGG", "CDC", "TTT", 'G', Blocks.glass, 'C', GCBlocks.oxygenCollector, 'D', Blocks.dirt, 'T', compressedTitaniumStack);

		// scale
		GameRegistry.addRecipe(ARBlocks.getItemStack(ARBlocks.blockScale, 1), "XXX", "ABC", "AAA", 'X', compressedTinStack, 'A', compressedSteelStack, 'B', waferAdvanced, 'C', Blocks.glass_pane);

		GameRegistry.addRecipe(ARItems.shuttleTank.getItemStack(1), "XXX", "XAX", "XXX", 'X', compressedTinStack, 'A', tinCanStack);

		GameRegistry.addRecipe(ARItems.dockGangway.getItemStack(1), "IXI", "IXI", "IXI", 'X', new ItemStack(Blocks.wool, 1, 15), // black wool
				'I', new ItemStack(GCItems.flagPole));

		GameRegistry.addRecipe(ARItems.dockDoor.getItemStack(1), "SSX", "SXS", "XSS", 'S', compressedSteelStack, 'X', compressedAluStack);

		GameRegistry.addRecipe(ARBlocks.getItemStack(ARBlocks.blockShuttleDock, 1), "XCX", "DAG", "XCX", 'D', ARItems.dockDoor.getItemStack(1), 'G', ARItems.dockGangway.getItemStack(1), 'A', Blocks.dropper, 'X', new ItemStack(GCBlocks.airLockFrame, 1, BlockAirLockFrame.METADATA_AIR_LOCK_FRAME), 'C', new ItemStack(GCBlocks.airLockFrame, 1, BlockAirLockFrame.METADATA_AIR_LOCK_CONTROLLER));

		GameRegistry.addRecipe(ARBlocks.getItemStack(ARBlocks.blockGravity, 1), "XBX", "XDA", "XWX", 'D', ARItems.darkShard.getItemStack(1), 'W', enderWaferStack, 'X', compressedTitaniumStack, 'B', beamCore, 'A', ARItems.transformer.getItemStack(1));
		...
		GameRegistry.addRecipe(ARBlocks.getItemStack(ARBlocks.blockObsidianBrick, 4), new Object[] { "XX", "XX", 'X', Blocks.obsidian });

		GameRegistry.addRecipe(ARBlocks.getItemStack(ARBlocks.blockAluCrate, 32), new Object[] { " X ", "X X", " X ", 'X', new ItemStack(GCItems.basicItem, 1, 8) // 8 = metadata for compressed alu
		});
		...
		GameRegistry.addShapelessRecipe(ARItems.uraniumIngot.getItemStack(9), ARBlocks.getItemStack(ARBlocks.blockUraniumBlock, 1));

		// nuclear generators
		// basic
		GameRegistry.addRecipe(new ShapedOreRecipe(ARBlocks.getItemStack(ARBlocks.blockIsotopeGeneratorBasic, 1), "XAX", "XBC", "XDX", 'X', compressedSteelStack, // compressed steel
				'A', waferAdvanced, 'B', "ingotUranium", 'C', new ItemStack(GCBlocks.aluminumWire, 1, 0), // basic wire
				'D', waferSolar));

		// advanced
		GameRegistry.addRecipe(new ShapedOreRecipe(ARBlocks.getItemStack(ARBlocks.blockIsotopeGeneratorAdvanced, 1), "XAX", "XBC", "XDX", 'X', compressedTitaniumStack, // compressed titanium
				'A', enderWaferStack, 'B', "blockUranium", 'C', new ItemStack(GCBlocks.aluminumWire, 1, 1), // basic wire
				'D', lithiumMeshStack));

		// **** chests ****
		GameRegistry.addRecipe(new ShapedOreRecipe(ARBlocks.getItemStack(ARBlocks.chestAlu, 1), "XXX", "X X", "XXX", 'X', "compressedAluminum"));

		GameRegistry.addRecipe(new ShapedOreRecipe(ARBlocks.getItemStack(ARBlocks.chestSteel, 1), "XXX", "X X", "XXX", 'X', "compressedSteel"));

		// **** mothership things ****

		// controller
		GameRegistry.addRecipe(new ShapedOreRecipe(ARBlocks.getItemStack(ARBlocks.blockMothershipController, 1), "XBX", "XAG", "XCX", 'A', freqModuleStack, // freq module here
				'B', enderWaferStack, 'C', AsteroidsItems.orionDrive, 'X', "compressedTitanium", 'G', Blocks.glass_pane));

		GameRegistry.addRecipe(new ShapedOreRecipe(ARBlocks.getItemStack(ARBlocks.blockMothershipSettings, 1), "XBX", "XAG", "XXC", 'A', freqModuleStack, 'B', enderWaferStack, 'C', Blocks.lever, 'X', "compressedTitanium", 'G', Blocks.glass_pane));

		// other stuff

		// thermal thingy
		GameRegistry.addRecipe(new ShapedOreRecipe(ARItems.thermalControl.getItemStack(2), " A ", "XBX", " C ", 'A', waferAdvanced, 'B', new ItemStack(Items.redstone, 1), 'C', GCItems.oxygenVent, 'X', new ItemStack(GCItems.canister, 1, 0)));

		// suit
		GameRegistry.addRecipe(new ShapedOreRecipe(ARItems.thermalHelm.getItemStack(1), "XAX", "X X", "   ", 'A', thermalControllerStack, 'X', thermalStuff));

		GameRegistry.addRecipe(new ShapedOreRecipe(ARItems.thermalChest.getItemStack(1), "X X", "XAX", "XAX", 'A', thermalControllerStack, 'X', thermalStuff));

		GameRegistry.addRecipe(new ShapedOreRecipe(ARItems.thermalLegs.getItemStack(1), "XXX", "A A", "X X", 'A', thermalControllerStack, 'X', thermalStuff));

		GameRegistry.addRecipe(new ShapedOreRecipe(ARItems.thermalBoots.getItemStack(1), "   ", "A A", "X X", 'A', thermalControllerStack, 'X', thermalStuff));

		GameRegistry.addRecipe(new ShapedOreRecipe(ARItems.tricorder.getItemStack(1), "XAX", "XBC", "XDE", 'X', compressedTinStack, 'A', freqModuleStack, 'B', waferAdvanced, 'C', Blocks.glass_pane, 'D', nuBattery, // ItemStack batteryFull = new ItemStack(GCItems.battery, 1, 0);
				'E', Blocks.stone_button));

		GameRegistry.addRecipe(new ShapedOreRecipe(ARItems.naniteControl.getItemStack(1), "XAX", "CBC", "XDX", 'X', compressedTinStack, 'C', freqModuleStack, 'B', beamCore, 'D', lithiumMeshStack, 'A', ARItems.naniteCluster.getItemStack(1)));

		GameRegistry.addRecipe(new ShapedOreRecipe(ARItems.gravityDisabler.getItemStack(1), "XEX", "RMR", "XBX", 'X', compressedAluStack, 'E', enderWaferStack, 'R', Items.redstone, 'M', ARItems.darkShard.getItemStack(1), 'B', nuBattery));

		ItemStack rocketBoosterTier1 = new ItemStack(GCItems.rocketEngine, 1, 1);

		// jet
		GameRegistry.addRecipe(new ShapedOreRecipe(ARBlocks.getItemStack(ARBlocks.blockMsEngineRocketJet, 1), "X X", " AB", "X X", 'A', new ItemStack(AsteroidsItems.basicItem, 1, 1), // heavy rocket engine here
				'B', rocketBoosterTier1, // tier 1 booster here
				'X', "compressedTitanium"));

		GameRegistry.addRecipe(new ShapedOreRecipe(ARBlocks.getItemStack(ARBlocks.blockMsEngineRocketBooster, 1), "XXX", "BCB", "XXX", 'B', rocketBoosterTier1, // tier 1 booster here
				'C', new ItemStack(GCItems.canister, 1, 0), // empty canister
				'X', "compressedTitanium"));

		// ionthruster
		GameRegistry.addRecipe(new ShapedOreRecipe(ARBlocks.getItemStack(ARBlocks.blockMsEngineIonJet, 1), "XXX", "BA ", "XXX", 'A', ARItems.goldFoil.getItemStack(1), 'B', beamCore, 'X', "compressedTitanium"));

		GameRegistry.addRecipe(new ShapedOreRecipe(ARBlocks.getItemStack(ARBlocks.blockMsEngineIonBooster, 1), "XCX", "BAB", "XXX", 'A', ARItems.goldFoil.getItemStack(1), 'B', beamCore, 'C', ARItems.transformer.getItemStack(1), 'X', "compressedTitanium"));

		// random misc items
		// shuttle legs
		GameRegistry.addRecipe(new ShapedOreRecipe(ARItems.shuttleLegs.getItemStack(1), "AXA", "X  ", "A  ", 'X', new ItemStack(GCItems.flagPole), 'A', "compressedTitanium"));

		// shuttle cone
		GameRegistry.addRecipe(new ShapedOreRecipe(ARItems.noseCone.getItemStack(1), " X ", "XAX", "   ", 'A', ARItems.lightPlating.getItemStack(1), 'X', "compressedTitanium"));

		// gold vent
		GameRegistry.addRecipe(new ShapedOreRecipe(ARItems.goldFoil.getItemStack(1), "XAX", "AXA", "XAX", 'A', compressedAluStack, 'X', "compressedGold"));

		// transformer
		GameRegistry.addRecipe(new ShapedOreRecipe(ARItems.transformer.getItemStack(1), "XAX", "XAX", "XAX", 'A', heavyWire, 'X', "compressedMeteoricIron"));
