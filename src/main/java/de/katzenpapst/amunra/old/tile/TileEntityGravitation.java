...
public class TileEntityGravitation extends TileBaseElectricBlock implements IInventory, IEnergyHandlerGC {
	...
	protected void doGravity() {
		...
		if (!world.isRemote) {
			final List<?> list = this.world.getEntitiesWithinAABB(Entity.class, box);
			...
			final List<?> list = this.world.getEntitiesWithinAABB(EntityPlayer.class, box);
			for (Object e : list) {
	...
	protected AxisAlignedBB getActualGravityBox() {
		...
		box = AxisAlignedBB.getBoundingBox(xCoord + box.minX, yCoord + box.minY, zCoord + box.minZ, xCoord + box.maxX + 1, yCoord + box.maxY + 1, zCoord + box.maxZ + 1);
		return box;
	}
