...
public abstract class TileEntityMothershipEngineAbstract extends TileBaseElectricBlockWithInventory implements ITileMothershipEngine, IFluidHandler, ISidedInventory, IInventory, ISoundableTile {
	...
	protected boolean attachBooster(int x, int y, int z, boolean notifyClient) {
		...
		Block worldBlock = this.world.getBlock(x, y, z);
		int worldMeta = this.world.getBlockMetadata(x, y, z);
		TileEntity worldTile = this.world.getTileEntity(x, y, z);
	...
	/**
	 * Should consume the fuel needed for the transition, on client side also start any animation or something alike. This will be called for all engines which are actually being used
	 */
	@Override
	...
	protected void checkBlocksInWay() {
		...
		for (int i = 0; i < exhaustCheckLength; i++) {
			...
			Block b = blockPos.getBlock(world);
			if (!b.isAir(world, blockPos.intX(), blockPos.intY(), blockPos.intZ())) {
				isObstructed = true;
				return;
			}
		}
	...
	protected void checkEntitiesInWay() {
		...
		final List<?> list = this.world.getEntitiesWithinAABB(EntityLivingBase.class, exhaustBB);
	...
	protected boolean detachBooster(int x, int y, int z, boolean notifyClient) {
		...
		if (!booster.getBlock().equals(worldBlock) || booster.getMetadata() != worldMeta || worldTile == null || !(worldTile instanceof TileEntityMothershipEngineBooster) || !((TileEntityMothershipEngineBooster) worldTile).isMaster(xCoord, yCoord, zCoord))
			return false;
	...
	public Vector3 getExhaustPosition(double scale) {
		double random1 = world.rand.nextGaussian() * 0.10F * scale;
		double random2 = world.rand.nextGaussian() * 0.10F * scale;
		double offset = 0.40D;
	...
	protected void processFluids() {
		...
		if (canister != null) {
			...
			/*
			 * if (this.containingItems[0].getItem() instanceof ItemCanisterGeneric) { if (this.containingItems[0].getItem() == GCItems.fuelCanister) { int originalDamage = this.containingItems[0].getItemDamage(); int used = this.fuelTank.fill(new FluidStack(GalacticraftCore.fluidFuel, ItemCanisterGeneric.EMPTY - originalDamage), true); if (originalDamage + used == ItemCanisterGeneric.EMPTY) this.containingItems[0] = new ItemStack(GCItems.oilCanister, 1, ItemCanisterGeneric.EMPTY); else
			 * this.containingItems[0] = new ItemStack(GCItems.fuelCanister, 1, originalDamage + used); } } else { final FluidStack liquid = FluidContainerRegistry.getFluidForFilledItem(this.containingItems[0]);
			 * 
			 * if (liquid != null) { boolean isFuel = FluidUtil.testFuel(FluidRegistry.getFluidName(liquid));
			 * 
			 * if (isFuel) { if (this.fuelTank.getFluid() == null || this.fuelTank.getFluid().amount + liquid.amount <= this.fuelTank.getCapacity()) { this.fuelTank.fill(new FluidStack(GalacticraftCore.fluidFuel, liquid.amount), true);
			 * 
			 * if (FluidContainerRegistry.isBucket(this.containingItems[0]) && FluidContainerRegistry.isFilledContainer(this.containingItems[0])) { final int amount = this.containingItems[0].stackSize; if (amount > 1) { this.fuelTank.fill(new FluidStack(GalacticraftCore.fluidFuel, (amount - 1) * FluidContainerRegistry.BUCKET_VOLUME), true); } this.containingItems[0] = new ItemStack(Items.bucket, amount); } else { this.containingItems[0].stackSize--;
			 * 
	...
	protected void spawnParticles() {
		 ...
		 * 
		 * AmunRa.proxy.spawnParticles(ParticleType.PT_MOTHERSHIP_JET_FLAME, this.world, particleStart, particleDirection); AmunRa.proxy.spawnParticles(ParticleType.PT_MOTHERSHIP_JET_FLAME, this.world, particleStart, particleDirection); AmunRa.proxy.spawnParticles(ParticleType.PT_MOTHERSHIP_JET_FLAME, this.world, particleStart, particleDirection); AmunRa.proxy.spawnParticles(ParticleType.PT_MOTHERSHIP_JET_FLAME, this.world, particleStart, particleDirection);
		 */
	}
	...
	protected void startSound() {
		...
		soundStarted = true;
		// AmunRa.proxy.playTileEntitySound(this, new ResourceLocation(Constants.TEXTURE_PREFIX + "shuttle.shuttle"));
	}
