...
public class TileEntityHydroponics extends TileEntityOxygen implements IPacketReceiver, IDisableableMachine, IInventory, ISidedInventory, IConnector {
	...
	public ItemStack[] getHarvest() {
		...
		int numSeeds = world.rand.nextInt(4);
	...
	/**
	 * Make sure this does not exceed the oxygen stored. This should return 0 if no oxygen is stored. Implementing tiles must respect this or you will generate infinite oxygen.
	 */
	@Override
	...
	/*
	 * @Override public Packet getDescriptionPacket() { NBTTagCompound data = new NBTTagCompound(); this.writeToNBT(data); return new S35PacketUpdateTileEntity(this.xCoord, this.yCoord, this.zCoord, 2, data); }
	 * 
	 * @Override public void onDataPacket(NetworkManager netManager, S35PacketUpdateTileEntity packet) { readFromNBT(packet.func_148857_g()); }
	 */
	...
	public void harvest(EntityPlayer player) {
		...
		for (ItemStack stack : harvest) {
			if (!player.inventory.addItemStackToInventory(stack)) {
				WorldHelper.dropItemInWorld(world, stack, player);
			}
		}
