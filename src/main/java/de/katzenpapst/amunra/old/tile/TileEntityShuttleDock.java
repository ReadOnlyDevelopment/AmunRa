...
public class TileEntityShuttleDock extends TileEntityAdvanced implements IFuelable, IFuelDock, ICargoEntity, IMultiBlock, IInventory, IPacketReceiver {
	...
	public TileEntityShuttleDock(String tileName) {
		super(tileName);
	...
	protected void checkTileAt(HashSet<ILandingPadAttachable> connectedTiles, int x, int y, int z) {
		...
		if (tile != null && tile instanceof ILandingPadAttachable && ((ILandingPadAttachable) tile).canAttachToLandingPad(this.world, this.xCoord, this.yCoord, this.zCoord)) {
	...
	protected void dockNearbyShuttle() {
		...
		final List<?> list = this.world.getEntitiesWithinAABB(EntityShuttle.class, AxisAlignedBB.getBoundingBox(expectedPosition.x - 0.5D, expectedPosition.y - 0.5D, expectedPosition.z - 0.5D, expectedPosition.x + 0.5D, expectedPosition.y + 0.5D, expectedPosition.z + 0.5D));
	...
	protected void dropItemsAtExit(List<ItemStack> cargo) {
		...
		for (final ItemStack item : cargo) {
			EntityItem itemEntity = new EntityItem(this.world, pos.x, pos.y, pos.z, item);
	...
	public boolean isAvailable() {
		if (this.dockedEntity != null || hasShuttleDocked)
			return false;
		return !isObstructed();
	}
	...
	public boolean onActivated(EntityPlayer entityPlayer) {
		entityPlayer.openGui(AmunRa.instance, GuiIds.GUI_SHUTTLE_DOCK, this.world, xCoord, yCoord, zCoord);
		return true;
	}
	...
	public void onCreate(BlockVec3 placedPosition) {
		...
		ARBlocks.metaBlockFake.makeFakeBlock(world, vecStrut, new BlockVec3(xCoord, yCoord, zCoord), ARBlocks.fakeBlockSealable);
	}
	...
	public void onDestroy(TileEntity callingBlock) {
		...
		if (b == ARBlocks.metaBlockFake) {
			this.world.setBlockToAir(xCoord, yCoord + 1, zCoord);
		}
		if (callingBlock != this) {
			// someone else called this, drop my actual block, too
			this.world.func_147480_a(this.xCoord, this.yCoord, this.zCoord, true);
	...
	public void performDockOperation(DockOperation op, EntityPlayerMP player) {
		...
		switch (op) {
			case DEPLOY_SHUTTLE:
				...
				shuttleEntity = shuttleItem.spawnRocketEntity(stack, world, pos.x, pos.y, pos.z);
