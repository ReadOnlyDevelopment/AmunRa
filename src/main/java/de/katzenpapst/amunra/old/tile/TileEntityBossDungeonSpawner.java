...
public class TileEntityBossDungeonSpawner extends TileEntityAdvanced implements ITileDungeonSpawner {
	...
	public void onBossDefeated() {
		...
		this.world.setBlockToAir(this.xCoord, this.yCoord, this.zCoord);
	}

	public void playSpawnSound(Entity entity) {
		this.world.playSoundAtEntity(entity, Constants.TEXTURE_PREFIX + "ambience.scaryscape", 9.0F, 1.4F);
	}
	...
	public void readFromNBT(NBTTagCompound nbt) {
		...
		/*
		 * if(nbt.hasKey("spawnedBoss")) { Entity ent = this.world.getEntityByID(nbt.getInteger("spawnedBoss")); if(ent != null && ent instanceof IAmunRaBoss) { boss = (IAmunRaBoss)ent; } }
		 */
	...
	public void updateEntity() {
		...
		if (!this.world.isRemote) {
			...
			List<Entity> entitiesInRoom = this.world.getEntitiesWithinAABB(EntityLivingBase.class, this.roomArea);
			int numPlayers = 0;
			...
			if (numPlayers > 0) {
				...
				if (this.boss == null && !this.isBossDefeated && !this.spawned) {
					...
					try {
						...
						this.boss = (IAmunRaBoss) c.newInstance(new Object[] { this.world });
						...
						this.spawned = true;
						isBossInRoom = true;
						this.world.spawnEntityInWorld((Entity) this.boss);
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
