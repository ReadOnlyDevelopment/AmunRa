...
import de.katzenpapst.amunra.old.block.BlockDoubleslabMeta;
import de.katzenpapst.amunra.old.block.BlockSlabMeta;
...
public class ItemSlabMulti extends ItemBlockMulti {
	...
	protected void combine(World world, ItemStack stack, int x, int y, int z, int meta) {
		if (world.checkNoEntityCollision(this.doubleSlab.getCollisionBoundingBoxFromPool(world, x, y, z)) && placeDoubleSlab(world, x, y, z, meta)) {
			world.playSoundEffect((double) (x + 0.5F), (double) (y + 0.5F), (double) (z + 0.5F), this.doubleSlab.stepSound.func_150496_b(), (this.doubleSlab.stepSound.getVolume() + 1.0F) / 2.0F, this.doubleSlab.stepSound.getPitch() * 0.8F);
			--stack.stackSize;
		}
	}
	...
	public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ) {
		if (this.isDoubleSlab)
			...
			if ((side == 1 && !isHighestBitSet || side == 0 && isHighestBitSet) && block == this.singleSlab && worldDistinctionMeta == stack.getItemDamage()) {
