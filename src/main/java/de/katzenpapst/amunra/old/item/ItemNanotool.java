...
public class ItemNanotool extends ItemAbstractBatteryUser implements ITool, IToolWrench {
	...
	protected boolean isEffectiveAgainst(Mode m, Block b) {
		...
		switch (m) {
			...
			case SHEARS:
				return b.getMaterial() == Material.leaves || b.getMaterial() == Material.cloth || b.getMaterial() == Material.carpet || b == Blocks.web || b == Blocks.redstone_wire || b == Blocks.tripwire;
			case SHOVEL:
		...
		if (this.hasEnoughEnergyAndMode(stack, energyCostUseSmall, Mode.HOE)) {
			...
			if (!player.canPlayerEdit(x, y, z, side, stack))
				...
				if (side != 0 && world.getBlock(x, y + 1, z).isAir(world, x, y + 1, z) && (block == Blocks.grass || block == Blocks.dirt)) {
					...
					world.playSoundEffect((double) (x + 0.5F), (double) (y + 0.5F), (double) (z + 0.5F), block1.stepSound.getStepResourcePath(), (block1.stepSound.getVolume() + 1.0F) / 2.0F, block1.stepSound.getPitch() * 0.8F);
