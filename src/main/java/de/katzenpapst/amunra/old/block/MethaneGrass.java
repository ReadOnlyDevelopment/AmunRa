...
public class MethaneGrass extends SubBlockGrass {
	...
	public boolean canLiveHere(World world, int x, int y, int z) {
		...
		return world.provider instanceof WorldProviderSpace && super.canLiveHere(world, x, y, z) && ((WorldProviderSpace) world.provider).isGasPresent(IAtmosphericGas.METHANE);
		// !OxygenUtil.testContactWithBreathableAir(world, world.getBlock(x, y+1, z), x, y, z, 0);
	}

	/**
	 * Return true if the conditions are right in order to spread to blocks returned by this.getDirtBlock() no call of canLiveHere is needed
	 *
	 * @param world
	 * @param x
	 * @param y
