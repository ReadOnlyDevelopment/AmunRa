		...
		for (int curY = -1; curY < height + 3; curY++) {
			for (int curX = (int) -outerRadius; curX <= +outerRadius; curX++) {
				for (int curZ = (int) -outerRadius; curZ <= +outerRadius; curZ++) {
					...
					if (Math.pow(curX / outerRadius, 2) + Math.pow((curY - centerOffset) / heightHalf, 2) + Math.pow(curZ / outerRadius, 2) <= 1) {
						// Block block1 = world.getBlock(curX+x, curY+y, curZ+z);

						if (curY > -1 && Math.pow(curX / innerRadius, 2) + Math.pow((curY - centerOffset) / innerHeightRadius, 2) + Math.pow(curZ / innerRadius, 2) < 1) {
							// inner stuff
						...
						/*
						 * if (block1.isAir(world, curX+x, curY+y, curZ+z) || block1.isLeaves(world, curX+x, curY+y, curZ+z)) { this.setBlockAndNotifyAdequately(world, curX+x, curY+y, curZ+z, this.wood.getBlock(), this.wood.getMetadata(), notify); }
						 */
