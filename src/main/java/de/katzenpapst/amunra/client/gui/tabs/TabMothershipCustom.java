...
public class TabMothershipCustom extends AbstractTab implements ITextBoxCallback, IMothershipSettingsTab {
	...
	public boolean actionPerformed(GuiButton btn) {
		/*
		 * if(btn.equals(applyButton)) { NBTTagCompound nbt = new NBTTagCompound (); nbt.setString("name", changedName); nbt.setString("bodyIcon", changedIcon.toString()); this.setGuiEnabled(false); AmunRa.packetPipeline.sendToServer(new PacketSimpleAR(EnumSimplePacket.S_SET_MOTHERSHIP_SETTINGS, ship.getID(), nbt)); return true; }
		 */
		/*
