...
public class GuiArtificialGravity extends GuiContainerGC implements ITextBoxCallback, ICheckBoxCallback {
	...
	private GuiElementInfoRegion electricInfoRegion = new GuiElementInfoRegion((this.width - this.xSize) / 2 + 112, (this.height - this.ySize) / 2 + 87, 52, 9, new ArrayList<String>(), this.width, this.height, this);
...
//          electricityDesc.add(EnumColor.YELLOW + GCCoreUtil.translate("gui.energyStorage.desc.1") + ((int) Math.floor(this.collector.getEnergyStoredGC()) + " / " + (int) Math.floor(this.collector.getMaxEnergyStoredGC())));
			...
			this.fontRendererObj.drawString(GCCoreUtil.translate("gui.message.status.name") + ": " + this.getStatus(), xOffset + 8, yOffset + 130, 4210752);

			this.fontRendererObj.drawString(GCCoreUtil.translate("gui.message.force.strength") + ": ", xOffset + 8, yOffset + 116, 4210752);
	...
	public void initGui() {
		...
		this.infoRegions.add(new GuiElementInfoRegion((this.width - this.xSize) / 2 + 152, (this.height - this.ySize) / 2 + 134, 18, 18, batterySlotDesc, this.width, this.height, this));
