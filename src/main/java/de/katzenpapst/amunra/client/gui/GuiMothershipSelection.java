...
public class GuiMothershipSelection extends GuiARCelestialSelection {
	...
	/*
	 * protected long getTravelTimeFor(CelestialBody body) { if(this.travelTimeCache.containsKey(body)) { return this.travelTimeCache.get(body); } TransitData tData = this.getTransitDataFor(body); double shipThrust = !tData.isEmpty() ? tData.thrust : provider.getTheoreticalTransitData().thrust;
	 * 
	 * double travelDistance = curMothership.getTravelDistanceTo(selectedBody);
	 * 
	 * long travelTime = AstronomyHelper.getTravelTimeAU(provider.getTotalMass(), shipThrust, travelDistance); this.travelTimeCache.put(body, travelTime); return travelTime; }
	 */
	...
	protected int drawFuelReqs(int offset, int mousePosX, int mousePosY, MothershipFuelRequirements fuelReqs) {
		...
		if (fuelReqs != null && !fuelReqs.isEmpty()) {
			// offset += 10;
			this.smallFontRenderer.drawSplitString(GCCoreUtil.translate("gui.message.mothership.fuelReqs") + ":", offsetX - 90, offsetY + offset, 90, ColorUtil.to32BitColor(255, 255, 255, 255));
			for (MothershipFuelDisplay f : fuelReqs.getData().keySet()) {
				...
				this.smallFontRenderer.drawSplitString(f.formatValue(fuelReqs.getData().get(f)), offsetX - 80, offsetY + offset, 90, ColorUtil.to32BitColor(255, 255, 255, 255));
			}
			offset += 10;
		}
		return offset;
	...
	protected void drawItemForFuel(int x, int y, ItemStack item) {
		...
		GuiCelestialSelection.itemRender.renderItemAndEffectIntoGUI(this.fontRendererObj, this.mc.renderEngine, item, (int) (x / factor), (int) (y / factor));
	...
	protected void drawMothershipGuiParts(int mousePosX, int mousePosY) {
		...
		/*
		 * int canCreateLength = Math.max(0, this.drawSplitString(GCCoreUtil.translate("gui.message.canCreateMothership.name"), 0, 0, 91, 0, true, true) - 2); int canCreateOffset = canCreateLength * this.smallFontRenderer.FONT_HEIGHT;
		 */
	...
	protected void drawMothershipInfo() {
		...
		this.drawTexturedModalRect(offsetX - boxWidth, bottomOffset - totalOffset, boxWidth, // displayed w
		...
		this.drawSplitString(this.curMothership.getLocalizedName(), offsetX - boxWidth / 2 - 2, // x?
				bottomOffset - totalOffset, // y
				91, // width?
				ColorUtil.to32BitColor(255, 255, 255, 255), false, false);

		/*
		 * protected final int RENAMEBUTTON_X = -95; protected final int RENAMEBUTTON_Y = 53; protected final int RENAMEBUTTON_W = 93; protected final int RENAMEBUTTON_H = 12;
		 */

		totalOffset -= 10;

		this.smallFontRenderer.drawString(GCCoreUtil.translate("gui.message.mothership.totalMass") + ": " + GuiHelper.formatKilogram(totalMass), offsetX - 90, bottomOffset - totalOffset, ColorUtil.to32BitColor(255, 255, 255, 255), false);

		totalOffset -= 10;

		this.smallFontRenderer.drawString(GCCoreUtil.translate("gui.message.mothership.totalBlocks") + ": " + GuiHelper.formatMetric(provider.getNumBlocks()), offsetX - 90, bottomOffset - totalOffset, ColorUtil.to32BitColor(255, 255, 255, 255), false);
		...
		this.smallFontRenderer.drawString(GCCoreUtil.translate("gui.message.mothership.travelThrust") + ": " + GuiHelper.formatMetric(tData.thrust, "N"), offsetX - 90, bottomOffset - totalOffset, thrustColor, false);
		...
		if (curMothership.isInTransit()) {
			...
			this.smallFontRenderer.drawString(GCCoreUtil.translate("gui.message.mothership.travelSpeed") + ": " + GuiHelper.formatSpeed(speed), offsetX - 90, bottomOffset - totalOffset, thrustColor, false);
		}
	}
	...
	protected void drawTargetBodyInfo(int mousePosX, int mousePosY) {
		...
		this.drawTexturedModalRect(offsetX + LAUNCHBUTTON_X, offsetY + LAUNCHBUTTON_Y, LAUNCHBUTTON_W, LAUNCHBUTTON_H, 0, // u
				4, // v
				LAUNCHBUTTON_W, LAUNCHBUTTON_H, false, false);

		this.drawSplitString(GCCoreUtil.translate("gui.message.mothership.launchbutton").toUpperCase(), offsetX - 48, offsetY + 135, 91, ColorUtil.to32BitColor(255, 255, 255, 255), false, false);

		// name of selected body
		offset = 17;
		this.drawSplitString(selectedBody.getLocalizedName(), offsetX - 48, offset + offsetY, 91, ColorUtil.to32BitColor(255, 255, 255, 255), false, false);
		...
		switch (failReason) {
			case ALREADY_ORBITING:
				this.smallFontRenderer.drawSplitString(GCCoreUtil.translate("gui.message.mothership.alreadyOrbiting"), offsetX - 90, offsetY + offset, 90, ColorUtil.to32BitColor(255, 255, 255, 255));
				offset += 10;
				break;
			case NONE:
			...
			case NOT_ENOUGH_FUEL:
				...
				offset = drawTravelTime(offset, travelDistance, travelTime);
				offset = drawFuelReqs(offset, mousePosX, mousePosY, tData.fuelReqData);
				this.smallFontRenderer.drawSplitString(GCCoreUtil.translate("gui.message.mothership.notEnoughFuel"), offsetX - 90, offsetY + offset, 90, ColorUtil.to32BitColor(255, 255, 128, 128));
				offset += 10;
				break;
			case NOT_ENOUGH_THRUST:
				...
				offset = drawTravelTime(offset, travelDistance, travelTime);
				offset = drawFuelReqs(offset, mousePosX, mousePosY, tData.fuelReqData);
				this.smallFontRenderer.drawSplitString(GCCoreUtil.translate("gui.message.mothership.notEnoughThrust"), offsetX - 90, offsetY + offset, 90, ColorUtil.to32BitColor(255, 255, 128, 128));
				offset += 10;
				break;
			case NOT_ORBITABLE:
				this.smallFontRenderer.drawSplitString(GCCoreUtil.translate("gui.message.mothership.unreachableBody"), offsetX - 90, offsetY + offset, 90, ColorUtil.to32BitColor(255, 255, 128, 128));
				offset += 10;
				break;
			case TRAVEL_TOO_LONG:
				...
				offset = drawTravelTime(offset, travelDistance, travelTime);
				offset = drawFuelReqs(offset, mousePosX, mousePosY, tData.fuelReqData);
				this.smallFontRenderer.drawSplitString(GCCoreUtil.translate("gui.message.mothership.travelTooLong"), offsetX - 90, offsetY + offset, 90, ColorUtil.to32BitColor(255, 255, 128, 128));
				offset += 10;
	...
	protected void drawTransitBar(int length) {
		// max length = 124
		this.drawTexturedModalRect(width / 2 - 90 + 27, height - 11 - GuiCelestialSelection.BORDER_WIDTH + 2, 1, // displayed w
				4, // displayed h
				0, // u aka x in tex
				0, // v
				1, // w in texture
				...
				false, false);
		// bar
		this.drawTexturedModalRect(width / 2 - 90 + 28, height - 11 - GuiCelestialSelection.BORDER_WIDTH + 2, length, // displayed w
				4, // displayed h
				1, // u aka x in tex
				0, // v
				43, // w in texture
				...
				false, false);
		// right cap
		this.drawTexturedModalRect(width / 2 - 90 + 28 + length, height - 11 - GuiCelestialSelection.BORDER_WIDTH + 2, 1, // displayed w
				4, // displayed h
				56, // u aka x in tex
				0, // v
				1, // w in texture
	...
	protected void drawTransitInfo(int mousePosX, int mousePosY) {
		...
		this.drawTexturedModalRect(width / 2 - 90, height - 11 - GuiCelestialSelection.BORDER_WIDTH - 4, TRANSIT_INFO_W, // w
				TRANSIT_INFO_H, // h
				TRANSIT_INFO_U, // u
				TRANSIT_INFO_V, // v
				TRANSIT_INFO_W, TRANSIT_INFO_H, false, false);
		...
		/*
		 * this.drawTexturedModalRect( width/2-90+28, height-11-GuiCelestialSelection.BORDER_WIDTH+2, length, //displayed w 4, //displayed h 1, // u aka x in tex 0, // v 43, // w in texture 4, // h in texture false, false);
		 */
	...
	protected int drawTravelDistance(int offset, double travelDistance) {
		// travel distance
		this.smallFontRenderer.drawString(GCCoreUtil.translate("gui.message.mothership.travelDistance") + ": " + GuiHelper.formatMetric(travelDistance, "AU"), offsetX - 90, offsetY + offset, ColorUtil.to32BitColor(255, 255, 255, 255), false);
		offset += 10;
		return offset;
	}
	...
	protected int drawTravelTime(int offset, double travelDistance, long travelTime) {
		...
		// travel time
		this.smallFontRenderer.drawString(GCCoreUtil.translate("gui.message.mothership.travelTime") + ": " + travelTimeStr, offsetX - 90, offsetY + offset, ColorUtil.to32BitColor(255, 255, 255, 255), false);
		return offset + 10;
	}
	...
	protected TravelFailReason getTravelFailReason(CelestialBody body, TransitDataWithDuration tData) {
		...
		if (tData.isEmpty())
			...
			/*
			 * float mass = provider.getTotalMass(); if(theoreticalData.thrust < mass) { return TravelFailReason.NOT_ENOUGH_THRUST; }
			 */
	...
	/*
	 * protected boolean canTravelTo(CelestialBody body, TransitData tData) { // simple stuff if ( body == null || body == curMothership.getParent() || !Mothership.canBeOrbited(body) ) { return false; } // more complicated stuff if(tData == null) { tData = getTransitDataFor(this.selectedBody); } if(tData.isEmpty()) { return false; } // most complicated stuff double distance = curMothership.getTravelDistanceTo(body); int travelTime = curMothership.getTravelTimeTo(distance, tData.speed);
	 * 
	 * 
	 * if(travelTime > AmunRa.instance.confMaxMothershipTravelTime) { return false; } return true; }
	 */
