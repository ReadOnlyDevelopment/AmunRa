...
public class GuiARCelestialSelection extends GuiCelestialSelection {
	...
	public static int getWidthForMothershipStatic(Mothership celestialBody) {
		...
		if (Minecraft.getMinecraft().currentScreen instanceof GuiShuttleSelection && (celestialBody != ((GuiShuttleSelection) Minecraft.getMinecraft().currentScreen).selectedBody || ((GuiShuttleSelection) Minecraft.getMinecraft().currentScreen).selectionCount != 1))
			// isScreenWtf = true;
			return 4;
		// return 6;
		return 6;
		/*
		 * 
		 * if(parent instanceof Star) { // like planet return isScreenWtf ? 4 : 6; } if(parent instanceof Planet) { // like moon return isScreenWtf ? 4 : 6; } if()
		 * 
	...
	public HashMap<CelestialBody, Matrix4f> drawCelestialBodies(Matrix4f worldMatrix) {
		...
		/*
		 * protected CelestialBody lastSelectedBodyMS; protected float mothershipsOrbit; protected int numMotherships; protected float mothershipPhase;
		 */
		...
		if (this.selectedBody != null) {
			...
			/*
			 * render them if: - renderShipsAround == lastSelectedBody - renderShipsAround's parent == lastSelectedBody - renderShipsAround == lastSelectedBody's parent
			 */
	...
	protected void drawMessageBox() {
		...
		int boxWidth = 179;
		int boxHeight = 67;
		this.drawTexturedModalRect((width - boxWidth) / 2, (height - boxHeight) / 2, boxWidth, boxHeight, 0, 135, // u/v position
		...
		messageButtonBox.setPositionSize((width - btnWidth) / 2, (height - btnHeight) / 2 + btnOffset, btnWidth, btnHeight);

		GL11.glColor4f(0.0F, 1.0F, 0.0F, 1);
		this.drawTexturedModalRect(messageButtonBox.minX, messageButtonBox.minY, messageButtonBox.getWidth(), messageButtonBox.getHeight(), 0, 202, // u/v position
				93, 12, // u/v size
				true, false);
		String str = messageCaption;
		this.fontRendererObj.drawString(str, (width - fontRendererObj.getStringWidth(str)) / 2, (height - boxHeight) / 2 + 3, ColorUtil.to32BitColor(255, 255, 255, 255));

		str = GCCoreUtil.translate("gui.message.mothership.okay");
		this.fontRendererObj.drawString(str, (width - fontRendererObj.getStringWidth(str)) / 2, (height - btnHeight) / 2 + 2 + btnOffset, ColorUtil.to32BitColor(255, 255, 255, 255));
		...
		this.fontRendererObj.drawSplitString(messageText, (width - boxWidth) / 2 + 4, (height - boxHeight) / 2 + 14, boxWidth - 8, ColorUtil.to32BitColor(255, 255, 255, 255));
	...
	protected List<Mothership> getMothershipListToRender() {
		...
		if (this.selectedBody != null) {
			...
			for (Mothership ms : msData.getMotherships().values()) {
				if ((ms == this.selectedBody || ms.getParent() == this.selectedBody && this.selectionCount != 1) && (this.ticksSinceSelection > 35 || this.selectedBody == ms || this.lastSelectedBody instanceof Mothership && ((Mothership) this.lastSelectedBody).getParent().equals(ms)) || isSiblingOf(this.selectedBody, ms)) {
					result.add(ms);
				}
			}
		}
	...
	protected Vector2f getTranslationAdvanced(float partialTicks) {
		/*
		 * if (this.selectedBody instanceof Planet && this.lastSelectedBody instanceof IChildBody && ((IChildBody) this.lastSelectedBody).getParentPlanet() == this.selectedBody) { Vector3f posVec = this.getCelestialBodyPosition(this.selectedBody); return new Vector2f(posVec.x, posVec.y); }
		 */
		if (this.selectedBody != null) {
	...
	protected void updatePlayerParent() {
		//
		CelestialBody body = ShuttleTeleportHelper.getCelestialBodyForDimensionID(this.mc.thePlayer.world.provider.dimensionId);
