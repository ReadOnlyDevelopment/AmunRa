...
public class GuiShuttleSelection extends GuiARCelestialSelection {
	...
	protected boolean canCreateMothership(CelestialBody atBody) {
		...
		return (AmunRa.config.maxNumMotherships == -1 || numPlayersMotherships < AmunRa.config.maxNumMotherships) && playerParent == selectedBody && Mothership.canBeOrbited(atBody);
	}
	...
	public void drawButtons(int mousePosX, int mousePosY) {
		...
		this.drawTexturedModalRect(exitBtnArea.minX, exitBtnArea.minY, exitBtnArea.getWidth(), exitBtnArea.getHeight(), 0, 392, 148, 22, true, true);
		String str = GCCoreUtil.translate("gui.message.cancel.name").toUpperCase();
		this.fontRendererObj.drawString(str, exitBtnArea.minX + (exitBtnArea.getWidth() - this.fontRendererObj.getStringWidth(str)) / 2, exitBtnArea.minY + 2, ColorUtil.to32BitColor(255, 255, 255, 255));
	}
	...
	protected void drawItemForRecipe(ItemStack item, int amount, int requiredAmount, int xPos, int yPos, int mousePosX, int mousePosY) {
		...
		GuiCelestialSelection.itemRender.renderItemAndEffectIntoGUI(this.fontRendererObj, this.mc.renderEngine, item, xPos, yPos);
		...
		this.smallFontRenderer.drawString(str, xPos + 8 - this.smallFontRenderer.getStringWidth(str) / 2,
	 ...
	 * 
	 * @Override protected int getAmountInInventory(ItemStack stack) { int amountInInv = super.getAmountInInventory(stack);
	 * 
	 ...
	 * //GCPlayerStats
	 * 
	 * if(rocket instanceof EntityAutoRocket) { EntityAutoRocket realRocket = (EntityAutoRocket)rocket;
	 * 
	 * for (int x = 0; x < realRocket.getSizeInventory(); x++) { final ItemStack slot = realRocket.getStackInSlot(x);
	 * 
	 * if (slot != null) { if (SpaceStationRecipe.checkItemEquals(stack, slot)) { amountInInv += slot.stackSize; } } }
	 * 
	 * }
	 * 
	...
	protected void drawMothershipButton(int mousePosX, int mousePosY) {
		...
		this.drawTexturedModalRect(width - GuiCelestialSelection.BORDER_WIDTH - GuiCelestialSelection.BORDER_EDGE_WIDTH - 95, // x
		...
		for (int barY = 0; barY < canCreateLength; ++barY) {
			this.drawTexturedModalRect(width - GuiCelestialSelection.BORDER_WIDTH - GuiCelestialSelection.BORDER_EDGE_WIDTH - 95, offset + GuiCelestialSelection.BORDER_WIDTH + GuiCelestialSelection.BORDER_EDGE_WIDTH + 138 + barY * this.smallFontRenderer.FONT_HEIGHT, 93, this.smallFontRenderer.FONT_HEIGHT, 159, 106, 93, this.smallFontRenderer.FONT_HEIGHT, false, false);
		}
		...
		if (recipe != null) {
			...
			buildMsBtnArea.setPositionSize(width - GuiCelestialSelection.BORDER_WIDTH - GuiCelestialSelection.BORDER_EDGE_WIDTH - 95, offset + GuiCelestialSelection.BORDER_WIDTH + GuiCelestialSelection.BORDER_EDGE_WIDTH + 182 + canCreateOffset, 93, 12);

			if (!this.mapMode) {
				if (buildMsBtnArea.isWithin(mousePosX, mousePosY)) {
					this.drawTexturedModalRect(buildMsBtnArea.minX, buildMsBtnArea.minY, buildMsBtnArea.getWidth(), buildMsBtnArea.getHeight(), 0, 174, 93, 12, false, false);
				}
			}
			...
			this.drawSplitString(GCCoreUtil.translate("gui.message.canCreateMothership.name"), width - GuiCelestialSelection.BORDER_WIDTH - GuiCelestialSelection.BORDER_EDGE_WIDTH - 48, offset + GuiCelestialSelection.BORDER_WIDTH + GuiCelestialSelection.BORDER_EDGE_WIDTH + 137, 91, ColorUtil.to32BitColor(255, color, 255, color), true, false);

			if (!mapMode) {
				this.drawSplitString(GCCoreUtil.translate("gui.message.createSS.name").toUpperCase(), width - GuiCelestialSelection.BORDER_WIDTH - GuiCelestialSelection.BORDER_EDGE_WIDTH - 48, offset + GuiCelestialSelection.BORDER_WIDTH + GuiCelestialSelection.BORDER_EDGE_WIDTH + 185 + canCreateOffset, 91, ColorUtil.to32BitColor(255, 255, 255, 255), false, false);
			}
		} // if (recipe != null)
		else {
			this.drawSplitString(GCCoreUtil.translate("gui.message.cannotCreateSpaceStation.name"), width - GuiCelestialSelection.BORDER_WIDTH - GuiCelestialSelection.BORDER_EDGE_WIDTH - 48, offset + GuiCelestialSelection.BORDER_WIDTH + GuiCelestialSelection.BORDER_EDGE_WIDTH + 138, 91, ColorUtil.to32BitColor(255, 255, 255, 255), true, false);
		}
				...
				if (this.selectedBody != null) {
					...
					if (recipe != null && this.canCreateMothership(this.selectedBody) && !createMothershipButtonDisabled) {
						if (recipe.matches(this.mc.thePlayer, false) || this.mc.thePlayer.capabilities.isCreativeMode) {
							...
							AmunRa.packetPipeline.sendToServer(new PacketSimpleAR(PacketSimpleAR.EnumSimplePacket.S_CREATE_MOTHERSHIP, new Object[] { AstronomyHelper.getOrbitableBodyName(this.selectedBody) }));
						}
						clickHandled = true;
					}
				}
		...
		if (this.selectedBody != null) {
			if (this.selectedBody.getReachable() && this.possibleBodies != null && this.possibleBodies.contains(this.selectedBody)) {
				try {
					...
					/*
					 * if (dimension.contains("$")) { this.mc.gameSettings.thirdPersonView = 0; } if(dimensionID == null) { return false; }
					 */
