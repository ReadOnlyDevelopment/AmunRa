...
public class GuiRocketEngine extends GuiContainerGC {
	...
	protected void drawGuiContainerBackgroundLayer(float var1, int var2, int var3) {
		...
		this.drawTexturedModalRect((this.width - this.xSize) / 2 + 8, // x
	...
	protected void drawGuiContainerForegroundLayer(int par1, int par2) {
		...
		displayString = GCCoreUtil.translate("gui.message.mothership.fuel") + ": " + GuiHelper.formatMetric(this.tileEngine.fuelTank.getFluidAmount() / 1000.0F, "B") + "/" + GuiHelper.formatMetric(tileEngine.fuelTank.getCapacity() / 1000.0F, "B");
	...
	public void initGui() {
		...
		fuelTankDesc.add("");
		tankInfo = new GuiElementInfoRegion((this.width - this.xSize) / 2 + 7, (this.height - this.ySize) / 2 + 27, 18, 76, fuelTankDesc, width, height, this);
		this.infoRegions.add(tankInfo);
		/*
		 * List<String> fuelTankDesc = new ArrayList<String>(); fuelTankDesc.add(GCCoreUtil.translate("gui.fuelTank.desc.2")); fuelTankDesc.add(GCCoreUtil.translate("gui.fuelTank.desc.3")); this.infoRegions.add( new GuiElementInfoRegion( (this.width - this.xSize) / 2 + 7, (this.height - this.ySize) / 2 + 33, 16, 38, fuelTankDesc, this.width, this.height, this ) );
		 */
	}
