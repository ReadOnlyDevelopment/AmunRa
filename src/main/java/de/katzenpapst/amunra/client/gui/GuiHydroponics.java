...
//          electricityDesc.add(EnumColor.YELLOW + GCCoreUtil.translate("gui.energyStorage.desc.1") + ((int) Math.floor(this.collector.getEnergyStoredGC()) + " / " + (int) Math.floor(this.collector.getMaxEnergyStoredGC())));
	...
	private String getStatus() {
		...
		/*
		 * if (returnValue.equals(EnumColor.DARK_GREEN + GCCoreUtil.translate("gui.status.active.name")) && this.tile.lastOxygenCollected <= 0.0F) { return EnumColor.DARK_RED + GCCoreUtil.translate("gui.status.missingleaves.name"); }
		 */
	...
	public void initGui() {
		...
		button = new GuiButton(0, (this.width - this.xSize) / 2 + 82, (this.height - this.ySize) / 2 + 88, 72, 20, GCCoreUtil.translate("tile.hydroponics.plant"));
