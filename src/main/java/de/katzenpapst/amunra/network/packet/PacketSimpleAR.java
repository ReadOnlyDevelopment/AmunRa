...
public class PacketSimpleAR implements IPacket {
	...
	public static enum EnumSimplePacket {
		 ...
		 */
		S_MOTHERSHIP_TRANSIT_START(LogicalSide.SERVER, Integer.class, String.class),

		/**
		 * Causes a mothership world provider to update itself (count total mass, etc) params: - dimension_id: dimension ID of the world
		 */
		S_MOTHERSHIP_UPDATE(Side.SERVER, Integer.class),

		/**
		 * Sends Mothership customisation parameters to the server params: - mothership_id: ID of the ship - nbt_data: subset of mothership data
		 */
		S_SET_MOTHERSHIP_SETTINGS(Side.SERVER, Integer.class, NBTTagCompound.class),

		/**
		 * Sends a username to the server and hopes that the server finds a user by that name params: - mothership_id - player_name - type
		 */
		...
		/**
		 * Updates the gravity side of an artifical gravity block - BlockVec3 pos: position of the block - BlockVec3 min: min of the AABB - BlockVec3 max: max of the AABB - Double gravityStrength
		 */
		S_ARTIFICIAL_GRAVITY_SETTINGS(Side.SERVER, BlockVec3.class, BlockVec3.class, BlockVec3.class, Double.class),

		// ===================== CLIENT =====================
		/**
		 * Signals the client that it has to open the shuttle GUI now params: - player_name: name of the player, is it really needed - dimension_list: copied over from GC, I should find a way to get rid of that
		 */
		C_OPEN_SHUTTLE_GUI(Side.CLIENT, String.class, String.class),

		/**
		 * Contains the list of motherships from the server, which the client has to replace his own list with params: - nbt_data: the nbt-encoded mothership list
		 */
		...
		/**
		 * Sent back to all clients, informing them that the transit has started params: - mothership_id: id of the ship - target_body_name: name of the target - travel_time: the travel time in ticks, as calculated by the server
		 */
		...
		/**
		 * Returns the data from a previous S_MOTHERSHIP_UPDATE to all clients in the dimension params: - dimension_id: the dimension id of the ship - nbt_data: the data to be read by the MothershipWorldProvider
		 */
		...
		/**
		 * Sends changed mothership setting to clients params: - mothership_id - nbt_data
		 */
		C_MOTHERSHIP_SETTINGS_CHANGED(Side.CLIENT, Integer.class, NBTTagCompound.class),

		/**
		 * Tells the client that a previous S_TELEPORT_SHUTTLE has failed because of a permission error params: - owner_name
		 */
		C_TELEPORT_SHUTTLE_PERMISSION_ERROR(Side.CLIENT, String.class),

		/**
		 * Tells the client that a previous S_TELEPORT_SHUTTLE has failed for any other random reason params: - error_msg
		 */
	...
	public void handleClientSide(EntityPlayer player) {
		...
		switch (this.type) {
			case C_OPEN_SHUTTLE_GUI:
				...
				if (String.valueOf(this.data.get(0)).equals(FMLClientHandler.instance().getClient().player.getGameProfile().getName())) {
					// TODO refactor
			...
			case C_MOTHERSHIP_DATA:
				...
				if (playerWorldProvider.getDimension() == dimId && playerWorldProvider instanceof MothershipWorldProvider) {
			...
			case C_MOTHERSHIP_SETTINGS_CHANGED:
				...
				if (player.world.provider.getDimension() == mShip.getDimensionID() && FMLClientHandler.instance().getClient().currentScreen instanceof GuiMothershipSettings) {
			...
			case C_TELEPORT_SHUTTLE_PERMISSION_ERROR:
				if (FMLClientHandler.instance().getClient().currentScreen instanceof GuiShuttleSelection) {
					...
					((GuiShuttleSelection) FMLClientHandler.instance().getClient().currentScreen).showMessageBox(GCCoreUtil.translate("gui.message.mothership.permissionError"), GCCoreUtil.translateWithFormat("gui.message.mothership.notAllowed", owner));
				}
				break;
			default:
				break;
		...
		switch (this.type) {
			case S_TELEPORT_SHUTTLE: // S_TELEPORT_ENTITY
				try {
					...
					if (playerBase.world instanceof WorldServer) {
						...
						if (mShip != null) {
							...
							if (playerBase.dimension != dim && !mShip.isPlayerLandingPermitted(playerBase)) {
								AmunRa.packetPipeline.sendTo(new PacketSimpleAR(PacketSimpleAR.EnumSimplePacket.C_TELEPORT_SHUTTLE_PERMISSION_ERROR, mShip.getOwner().getName()), playerBase);
								return;
							}
						}

						world = (WorldServer) playerBase.world;
						// replace this now
						ShuttleTeleportHelper.transferEntityToDimension(playerBase, dim, world);

						stats.setTeleportCooldown(10);
						GalacticraftCore.packetPipeline.sendTo(new PacketSimple(PacketSimple.EnumSimplePacket.C_CLOSE_GUI, dim, new Object[] {}), playerBase);
			...
			case S_CANCEL_SHUTTLE:
				if (playerBase.world instanceof WorldServer) {
					if (playerBase.getRidingEntity() != null && playerBase.getRidingEntity() instanceof EntityShuttleFake) {
						// player is actually in the sky
						world = (WorldServer) playerBase.world;
						ShuttleTeleportHelper.transferEntityToDimension(playerBase, world.provider.getDimension(), world);
						stats.setTeleportCooldown(10);
					}
					GalacticraftCore.packetPipeline.sendTo(new PacketSimple(PacketSimple.EnumSimplePacket.C_CLOSE_GUI, op, new Object[] {}), playerBase);
				}
				break;
			case S_CREATE_MOTHERSHIP:
				...
				if (Mothership.canBeOrbited(targetBody) && (AmunRa.config.maxNumMotherships < 0 || TickHandlerServer.mothershipData.getNumMothershipsForPlayer(playerBase) < AmunRa.config.maxNumMotherships)) {
			...
			case S_DOCK_OPERATION:
				...
				tileEntity = playerBase.world.getTileEntity(new BlockPos(x, y, z));
			...
			case S_ARTIFICIAL_GRAVITY_SETTINGS:
				...
				if (tileEntity instanceof TileEntityGravitation) {
					...
					playerBase.world.markBlockForUpdate(tileEntity.xCoord, tileEntity.yCoord, tileEntity.zCoord);
				}
				break;
			default:
				break;
	...
	public void processPacket(INetHandler var1) {
		/*
		 * if (this.type != EnumSimplePacket.C_UPDATE_SPACESTATION_LIST && this.type != EnumSimplePacket.C_UPDATE_PLANETS_LIST && this.type != EnumSimplePacket.C_UPDATE_CONFIGS) { return; }
		 */
