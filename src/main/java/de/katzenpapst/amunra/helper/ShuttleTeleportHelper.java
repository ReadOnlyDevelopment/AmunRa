...
public class ShuttleTeleportHelper {
	...
	public static HashMap<String, Integer> getArrayOfPossibleDimensions(EntityPlayerMP playerBase) {
		...
		if (playerBody == null) {
			...
			final SpaceStationWorldData data = SpaceStationWorldData.getStationData(playerBase.world, playerBase.dimension, null);
			if (data != null) {
			...
			/*
			 * playerBody = GalacticraftCore.satelliteSpaceStation; HashMap<String, Integer> map = new HashMap<String, Integer>();
			 * 
			 * 
			 * final SpaceStationWorldData data = SpaceStationWorldData.getStationData(playerBase.world, playerBase.dimension, null); map.put(playerBody.getName() + "$" + data.getOwner() + "$" + data.getSpaceStationName() + "$" + playerBase.dimension + "$" + data.getHomePlanet(), playerBase.dimension);
			 * 
			 * // TEMP! CelestialBody parent = ((Satellite)playerBody).getParentPlanet(); map.putAll(getArrayOfChildren(playerBase, parent));
			 * 
			 * return map;
			 */
	...
	private static void landInShuttle(World world, EntityPlayerMP player, ItemShuttle item, Vector3 spawnPos) {
		...
		EntityShuttle shuttle = item.spawnRocketEntity(new ItemStack(playerStats.rocketItem, 1, playerStats.rocketType), world, spawnPos.x, spawnPos.y, spawnPos.z);
		...
		if (playerStats.launchpadStack != null && playerStats.launchpadStack.stackSize > 0 && playerStats.launchpadStack.getItem() != null) {
			...
			if (shuttle.rocketType.getInventorySpace() <= 2) {
				...
				if (!player.inventory.addItemStackToInventory(playerStats.launchpadStack)) {
					...
					/*
					 * EntityItem itemEntity = new EntityItem(world, itemDropPosition.x, itemDropPosition.y, itemDropPosition.z, playerStats.launchpadStack); world.spawnEntityInWorld(itemEntity);
					 */
				}
	...
	private static Entity teleportEntity(World worldNew, Entity entity, int dimID, ITeleportType type) {
		...
		int oldDimID = entity.world.provider.dimensionId;
		...
		if (dimChange) {
			if (entity instanceof EntityPlayerMP) {
				...
				player.playerNetServerHandler.sendPacket(new S07PacketRespawn(dimID, player.world.difficultySetting, player.world.getWorldInfo().getTerrainType(), player.theItemInWorldManager.getGameType()));
				...
				spawnPos = type.getPlayerSpawnLocation((WorldServer) entity.world, player);
		...
		if (!usingShuttle) {
			...
			if (playerStats.rocketStacks != null && playerStats.rocketStacks.length > 0) {
				...
				if (playerStats.chestSpawnCooldown == 0) {
					playerStats.chestSpawnVector = type.getParaChestSpawnLocation((WorldServer) entity.world, player, new Random());
