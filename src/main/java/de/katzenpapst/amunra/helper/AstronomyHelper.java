...
public class AstronomyHelper {
	...
	/**
	 * Finds the closest common parent of the given bodies. For two moons, it should be their parent planet, for a planet and it's moon the planet, for two planets in the same solar system or two moons of different planetes in the same system the star, for objects of different systems null.
	 * 
	 * @param body1
	 * @param body2
	 * @return
	...
	/**
	 * Calculates the distance between two bodies from their distances from the sun/parent planet and the angle between them
	 *
	 * @param innerAngle
	 * @param body1distance
	 * @param body2distance
	...
	public static double getDistanceBetweenBodies(double innerAngle, double body1distance, double body2distance) {
		return Math.sqrt(Math.pow(body2distance, 2) + Math.pow(body1distance, 2) - 2 * body2distance * body1distance * Math.cos(innerAngle));
	}
	...
	public static AngleDistance projectBodyToSky(CelestialBody curBody, CelestialBody otherBody, float partialTicks, long worldTime) {
		...
		double curBodyOrbitalAngle = getOrbitalAngle(curBodyDist, curBody.getPhaseShift(), worldTime, partialTicks, yearFactor);

		double otherBodyOrbitalAngle = getOrbitalAngle(otherBody.getRelativeOrbitTime(), otherBody.getPhaseShift(), worldTime, partialTicks, yearFactor);
