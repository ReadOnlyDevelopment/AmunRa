...
public class ModelRobotVillager extends ModelVillager {
	...
	public ModelRobotVillager(float scaleOrSo, float par2, int textureX, int textureY) {
		...
		/*
		 * visor = new ModelRenderer(this).setTextureSize(textureX, textureY); visor.setRotationPoint(0.0F, par2 - 4.0F, 0.0F); visor.setTextureOffset(40, 0).addBox(-4.0F, -1.5F, -5.0F, 8, 3, 0, scaleOrSo);
		 * 
	...
	public void setRotationAngles(float time, float walkSpeed, float appendageRotation, float rotationYaw, float rotationPitch, float scale, Entity p_78087_7_) {
		...
		/*
		 * this.villagerArms.rotationPointY = 3.0F; this.villagerArms.rotationPointZ = -1.0F; this.villagerArms.rotateAngleX = -0.75F;
		 */
	...
	/*
	 * private void animateVisor(float partialTicks) { ticks += partialTicks;
	 * 
	 * 
	 * 
	 * if(ticks > 12) {
	 * 
	 * ticks = ticks - ((int)(ticks/12))*12; } int yOffset = (int)(ticks); if(curVisorPhase != yOffset) { curVisorPhase = yOffset;
	 * 
	 * // update rendering if(curVisorPhase < 6) { visor.setTextureOffset(40, curVisorPhase*3); } else { visor.setTextureOffset(40, (6-curVisorPhase)*3); } } //if(ticks <= 0) { // visor.setTextureOffset(40, 0); //} }
	 */
