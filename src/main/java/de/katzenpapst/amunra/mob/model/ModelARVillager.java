...
public class ModelARVillager extends ModelVillager {
	...
	public ModelARVillager(float scaleOrSo, float par2, int textureX, int textureY) {
		...
		for (int i = 0; i < tailBoxes.length; i++) {
			...
			/*
			 * if(i < tailBoxes.length-1) { curOffset += (tailBoxSizes[i+1] * 2); } if(i > 0) { curOffset += (tailBoxSizes[i-1]); }
			 */
			// curOffset += (tailBoxSizes[i]);
			tailBoxes[i].setTextureOffset(curData[0], curData[1]).addBox(-halfSize, -curSize + 12.0F, curPos, curSize, curSize, curSize);
			// curOffset += (2 + curSize);
			// tailBoxes[i].s
		}
	...
	public void render(Entity curEntity, float limbSwingTime, float limbSwingAmplitude, float totalTimeMaybe, float rotationY, float rotationX, float someConstant) {
		...
		for (ModelRenderer tailBoxe : tailBoxes) {
			tailBoxe.render(someConstant);
		}
