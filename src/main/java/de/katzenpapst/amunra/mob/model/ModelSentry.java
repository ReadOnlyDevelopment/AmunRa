...
public class ModelSentry extends ModelBase {
	...
	public ModelSentry() {
		/*
		 * // see net.minecraft.client.model.ModelGhast byte heightOffset = -16; this.body = new ModelRenderer(this, 0, 0); this.body.addBox(-8.0F, -8.0F, -8.0F, 16, 16, 16); // 16³ box this.body.rotationPointY += (float)(24 + heightOffset); // why 24?
		 */
	...
	public void render(Entity entity, float limbSwingTime, float limbSwingAmplitude, float totalTimeMaybe, float rotationY, float rotationX, float someConstant) {
		...
		/*
		 * leftarm.render(someConstant); rightarm.render(someConstant); toparm.render(someConstant); bottomarm.render(someConstant);
		 */
		GL11.glPopMatrix();
	}
	...
	public void setRotationAngles(float limbSwingTime, float linbSwingAmount, float somethingWhatever, float rotationY, float rotationX, float p_78087_6_, Entity p_78087_7_) {
		...
		rightarm.rotateAngleY = 0.2F * MathHelper.sin(somethingWhatever * 0.3F) + 0.4F;
		leftarm.rotateAngleY = -(0.2F * MathHelper.sin(somethingWhatever * 0.3F) + 0.4F);
