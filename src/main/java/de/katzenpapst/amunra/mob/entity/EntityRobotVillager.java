...
public class EntityRobotVillager extends EntityAgeable implements IEntityBreathable, INpc, IMerchant {
	...
	/**
	 * based on the villagers profession add items, equipment, and recipies adds par1 random items to the list of things that the villager wants to buy. (at most 1 of each wanted type is added)
	 */
	private void addDefaultEquipmentAndRecipies(int p_70950_1_) {
		...
		switch (baseList.size()) {
			...
			case 1:
				...
				break;
			default:
				// int numOffers = world.rand.nextInt(baseList.size());
				// for now have just 1 offer
				int numOffers = world.rand.nextInt(baseList.size() - 1) + 1;// ensure it's at least 1
				HashMap<Integer, Boolean> uniqCache = new HashMap<Integer, Boolean>();
				for (int i = 0; i < numOffers; i++) {
					int randOffer = world.rand.nextInt(baseList.size());
		...
		/*
		 * 
		 * MerchantRecipeList merchantrecipelist; merchantrecipelist = new MerchantRecipeList(); VillagerRegistry.manageVillagerTrades(merchantrecipelist, this, this.getProfession(), this.rand); int k;
		 * 
		 * 
		 * 
		 * if (merchantrecipelist.isEmpty()) { func_146091_a(merchantrecipelist, Items.gold_ingot, this.rand, 1.0F); }
		 * 
		 * Collections.shuffle(merchantrecipelist);
		 * 
		 * if (this.buyingList == null) { this.buyingList = new MerchantRecipeList(); }
		 * 
		 * for (int l = 0; l < p_70950_1_ && l < merchantrecipelist.size(); ++l) { this.buyingList.addToListWithCheck((MerchantRecipe)merchantrecipelist.get(l)); }
		 */
	}
	...
	public EntityRobotVillager func_90012_b(EntityAgeable par1EntityAgeable) {
		return new EntityRobotVillager(this.world);
	}
	...
	public IEntityLivingData onSpawnWithEgg(IEntityLivingData p_110161_1_) {
		...
		this.setProfession(RobotVillagerProfession.getRandomProfession(world.rand));
		// VillagerRegistry.applyRandomTrade(this, world.rand);
		return p_110161_1_;
	}
		...
		if (!this.isTrading() && this.timeUntilReset > 0) {
			...
			if (this.timeUntilReset <= 0) {
				if (this.needsInit) {
					...
					/*
					 * if (this.villageObj != null && this.lastBuyingPlayer != null) { this.world.setEntityState(this, (byte)14); this.villageObj.setReputationForPlayer(this.lastBuyingPlayer, 1); }
					 */
				}
