...
public class MothershipWorldData extends WorldSavedData {
	...
	public Mothership addMothership(Mothership ship) {
		...
		if (MCUtil.getServer() != null && !MCUtil.getServer().isDedicatedServer()) {
	...
	public Mothership registerNewMothership(EntityPlayer player, CelestialBody currentParent) {
		...
		AmunRa.packetPipeline.sendToAll(new PacketSimpleAR(PacketSimpleAR.EnumSimplePacket.C_NEW_MOTHERSHIP_CREATED, new Object[] { data }));
		return ship;
	}
	...
	public void tickAllMotherships() {
		...
		if (hasChanged || !hasChanged && numTicksWithoutSave > 0) {
			...
			/*
			 * if(hasChanged) { NBTTagCompound data = new NBTTagCompound (); TickHandlerServer.mothershipData.writeToNBT(data); AmunRa.packetPipeline.sendToAll(new PacketSimpleAR(PacketSimpleAR.EnumSimplePacket.C_UPDATE_MOTHERSHIP_LIST, data)); }
			 */
		}
	}
	 ...
	 */
	/*
	 * public void updateFromNBT(NBTTagCompound data) { NBTTagList tagList = data.getTagList("MothershipList", 10);
	 * 
	 * for (int i = 0; i < tagList.tagCount(); i++) { NBTTagCompound mothershipNBT = tagList.getCompoundTagAt(i); int id = mothershipNBT.getInteger("id"); Mothership m = this.getByMothershipId(id); if(m != null) { m.updateFromNBT(mothershipNBT); } else { m = Mothership.createFromNBT(mothershipNBT); if(highestId < id) { highestId = id; } if(DimensionManager.isDimensionRegistered(m.getDimensionID())) { if(DimensionManager.getProviderType(m.getDimensionID()) != AmunRa.config.mothershipProviderID) {
	 * // now that shouldn't happen throw new RuntimeException("Dimension "+m.getDimensionID()+" should be registered for an AmunRa Mothership, registered for "+DimensionManager.getProviderType(m.getDimensionID())+" instead"); } // it's fine otherwise } else { DimensionManager.registerDimension(m.getDimensionID(), AmunRa.config.mothershipProviderID); }
	 * 
