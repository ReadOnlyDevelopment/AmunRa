...
import de.katzenpapst.amunra.old.block.machine.mothershipEngine.MothershipEngineJetBase;
import de.katzenpapst.amunra.old.tile.ITileMothershipEngine;
...
public class MothershipWorldProvider extends WorldProviderSpace implements IZeroGDimension, ISolarLevel, IExitHeight {
	...
	/**
	 * On client, this is the sole authority regarding the day length On server, this is changed and sent to client as needed
	 */
	...
	protected TransitData calcTheoreticalTransitData() {
		...
		for (Vector3int loc : engineLocations) {
			TileEntity tile = this.world.getTileEntity(loc.x, loc.y, loc.z);
			if (tile instanceof ITileMothershipEngine) {
				...
				// double curSpeed = engine.getSpeed(world, loc.x, loc.y, loc.z, meta);
				double curThrust = engine.getThrust();
				if (curThrust <= 0) {
					continue; // not sure when this could happen, but in that case, this engine doesn't count
				}
				/*
				 * if(tDatas[direction].speed == -1 || curSpeed < tDatas[direction].speed) { tDatas[direction].speed = curSpeed; }
				 */
	...
	public void endTransit() {
		...
		for (Vector3int loc : this.engineLocations) {
			TileEntity t = this.world.getTileEntity(loc.x, loc.y, loc.z);
	...
	/**
	 * This should process one block for the mothership update, figuring out it's mass and stuff The coordinates must be world-global, not chunk-local
	 *
	 * @param block
	 * @param meta
	 * @param x
	...
	protected void processBlock(Block block, int meta, int x, int y, int z) {
		// first, the mass
		float m = BlockMassHelper.getBlockMass(this.world, block, meta, x, y, z);
	...
	public void readFromNBT(NBTTagCompound nbt) {
		...
		for (int i = 0; i < list.tagCount(); i++) {
			...
			Vector3int pos = new Vector3int(posData.getInteger("x"), posData.getInteger("y"), posData.getInteger("z"));
	...
	protected void saveData(boolean notifyClients) {
		if (mothershipSaveFile == null) {
			mothershipSaveFile = MothershipWorldProviderSaveFile.getSaveFile(world);
		}
	...
	/**
	 * Does the last minute MS check and starts the transit here and in the MS object. Returns true if this worked This should only ever happen on server
	 * 
	...
	public void updateWeather() {
		...
		if (!this.world.isRemote) {
			if (!hasLoadedWorldData) {
				...
				this.mothershipSaveFile = MothershipWorldProviderSaveFile.getSaveFile(world);
