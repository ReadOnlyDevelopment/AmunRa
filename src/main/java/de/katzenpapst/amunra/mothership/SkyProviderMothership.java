...
public class SkyProviderMothership extends SkyProviderDynamic {
	...
	/*
	 * @Override protected void prepareSystemForRender(long curWorldTime, float partialTicks) { super.prepareSystemForRender(curWorldTime, partialTicks); renderMothershipParent(); }
	 */

	/*
	 * protected void renderMothershipParent() { double distanceToParent = 1;
	 * 
	 * 
	 * float zIndex = (float) (20/distanceToParent); float distance = (float) (curBodyPlanet.getRelativeSize() / distanceToParent); // my parent this.nearBodiesToRender.add( new BodyRenderTask(curBodyPlanet, 0, zIndex, distance, (float) Math.PI) ); }
	 */
	...
	protected void initStarLines(int list, double radius) {
		...
		for (int starIndex = 0; starIndex < 400; ++starIndex) {
			...
			// paint
			// A
			tess.getBuffer().sortVertexData(x1, y1, zBase);
			// B
