...
import de.katzenpapst.amunra.old.item.SubItemToggle;
...
public class ClientProxy extends ARSidedProxy {
	...
	/**
	 * +0,08 seems to be 1g -0,054 seems to be -1g I'll assume that 5 = 1g 5 in gui => 0,05 in vector
	 */
	...
	public static void registerItemRenderers() {
		...
		MinecraftForgeClient.registerItemRenderer(Item.getItemFromBlock(ARBlocks.metaBlockMothershipEngineJet), new ItemRendererJet(new IModelCustom[] { engineModel, engineModelIon }, new ResourceLocation[] { new ResourceLocation(AmunRa.ASSETPREFIX, "textures/model/jet.png"), new ResourceLocation(AmunRa.ASSETPREFIX, "textures/model/jet-ion.png") }));
	...
	public void handlePlayerArtificalGravity(EntityPlayer player, double gravity) {
		if (player instanceof EntityPlayerSP) {
			...
			/*
			 * if(ARItems.gravityDisabler.isSameItem(p.getEquipmentInSlot(0))) { return; }
			 */

			TickHandlerClient.playerGravityState = 2;

			// hack against the jumping bug in 498
			if (p.world.provider instanceof IZeroGDimension) {
	...
	public void preInit(FMLPreInitializationEvent event) {
		...
		/*
		 * GalacticraftPlanets.clientModules.put(GalacticraftPlanets.MODULE_KEY_MARS, new MarsModuleClient()); GalacticraftPlanets.clientModules.put(GalacticraftPlanets.MODULE_KEY_ASTEROIDS, new AsteroidsModuleClient());
		 * 
		 * super.preInit(event);
		 * 
		 * for (IPlanetsModuleClient module : GalacticraftPlanets.clientModules.values()) { module.preInit(event); }
		 */
	}
